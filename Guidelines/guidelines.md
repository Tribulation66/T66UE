...
## 13. Material & Lighting Standards

### Shading Model: ALL materials must be Unlit

Chadpocalypse uses a fully Unlit rendering approach. Every material in the game — characters, enemies, NPCs, companions, environment, props, structures, VFX — must use the **Unlit** shading model. No material should respond to directional lights, sky lights, or any dynamic lighting.

**Why:** The game's visual style (inspired by ULTRAKILL and Megabonk) requires that all objects display their textures at full brightness from every angle. There should be no bright side / dark side on any mesh based on light direction. The directional light only affects the Sky Atmosphere for the eclipse dusk sky effect.

### Master Materials

All material instances must inherit from one of these Unlit master materials:
...
The scene lighting exists only for:
- **Sky Atmosphere**: Creates the red eclipse dusk sky (Rayleigh scattering)
- **Eclipse Actor**: Visual eclipse effect in the sky (AT66EclipseActor)
- **Posterize Post-Process**: Sky-only color banding for the retro look
