...
- **Fresh prop reimport utility:** `Scripts/RunImportPropsAndExit.py` now wraps `ImportStaticMeshes.py` and executes only the `Props/` manifest entries through `UnrealEditor.exe`, then quits. This is the clean path for rerunning just `Tree2`, `Grass`, `Rock`, `Bush`, and `Boulder` after importer fixes without touching bags/fountain.
- **Import batch verification added:** `Scripts/VerifyImportBatch.py` validates the current prop/interactable/goblin batch and writes `Saved/Logs/VerifyImportBatch.json`. `Scripts/RepairStaticMeshImportBatch.py` re-runs the GLB repair pass without reimporting. Important nuance: flattened GLB meshes may live at `/Game/.../AssetName` while their imported materials live under `/Game/.../AssetName/Materials`, so repair/verification must scan both roots and rebind the flattened mesh material slots. `MakeGLBImportsUnlit.py` now accepts either `BaseColorTexture` or fallback `DiffuseColorMap` when converting GLB imports to `M_GLB_Unlit`.
...
- **Current import batch gameplay changes:** `Tree2`, `Grass`, and `Rock` are set up to be replaced by new GLB imports; `Bush` and `Boulder` were added to `Props.csv`; loot bags now point at the new black/red/yellow/white bag import destinations; goblin visuals now use imported FBX rows in `CharacterVisuals.csv`.
