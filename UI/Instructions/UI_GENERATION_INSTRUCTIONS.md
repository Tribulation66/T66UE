...
Reference images and proof captures:

```text
C:\UE\T66\Saved\Codex\UI\<ScreenName>\
```

Generated review files are temporary and can be deleted after the accepted runtime assets and process notes are promoted. Historical UI output roots from the pre-cleanup repo are recoverable from `v4.1` if needed.

`RuntimeDependencies\T66\UI\Reference\Shared` is a bootstrap and compatibility source, not the destination for newly accepted screen-specific work. Before a target is considered individualized, any asset it uses from Shared or from another screen/modal must be duplicated into that target's folder and renamed for that target.
...
- Route accepted target assets from that target's own folder under `RuntimeDependencies\T66\UI\Reference\Screens` or `RuntimeDependencies\T66\UI\Reference\Modals`.
...
- If the reference contains a visible button, row, skin, slot, panel, or content state that has no backend/gameplay infrastructure yet, build the visual element as a clearly marked stub so frontend fidelity is not blocked by backend sequencing.
- Individual target iteration uses loose runtime assets from `RuntimeDependencies\T66\UI\Reference\...` and the shared staged standalone executable.
...
- `Source/T66/T66.Build.cs` stages `RuntimeDependencies/T66/UI/Reference/...` as loose runtime UI assets.
...
3. Compare the working capture against the reference image with `C:\UE\T66\Scripts\CompareUIScreen.ps1`.
...
- No completion claim without an existing working screenshot comparison artifact from `C:\UE\T66\Scripts\CompareUIScreen.ps1`.
