...
Common patterns:
- A row of related buttons (top bar, sub-tab row) may have a single outer purple container wrapping loose buttons inside, OR be a flat row of loose buttons with no outer container. Each screen specifies which.
...
- **Default** — dark fill, dark-purple structural border, light text. No glow. The "normal" state of any interactive element.
...
- Notches, tabs, angular cuts on panel edges
- Rounded corners (sharp 90° only)
- Diagonal cuts, chamfers, asymmetric shapes
- Scanlines, CRT/glitch effects, chromatic aberration on chrome (note: the world-facing retro FX subsystem can still apply effects to the game world; flat UI chrome is excluded from that pass via the bypass mechanism, see Stage 1 helper spec)
- Background noise, grain, texture, pattern fills inside chrome (content artwork — hero portraits, 3D renders, diploma art — retains its native rendering; this rule applies to chrome only)
...
- Render the tab as normal (Default purple or Selected red)
- Add a small lowercase-"i" glyph inside the tab button, positioned to the right of the tab label, sized roughly to the tab text's cap height, colored matching the tab's state (red for selected, purple text accent for default)
...
**Card row pattern** — Diplomas, Drugs, Minigames. A row of self-contained cards inside an outer purple container, each card containing artwork + name + description + a red-bordered action button.
...
- `MakeFlatStatRow(label_text, value_text, label_role, value_role)` — single label/value row with thin bottom divider in dim purple.
- `MakeFlatStatsTable(rows, columns)` — multi-column stat grid with thin row dividers and no full cell borders.
- `MakeFlatProgressBar(percent, optional_color)` — horizontal bar with purple track, red fill (default) or green fill (when used for Ready/positive). No glow.
...
```
Reference docs:
- C:\UE\T66\UI\T66_UI_FLAT_REDESIGN_PLAN.md (this master plan)
...
- State assignments (which buttons are Selected red, which are Default purple, etc.)
...
- Two-column body:
  - LEFT (no outer container): three stacked sub-panels.
    - Player block (purple-bordered): borderless avatar slot left + right stack of [RandomChad name / LEVEL 1/100 in red text / EXPERIENCE label with XP value right-aligned / red progress bar].
    - Account Status (purple-bordered): shield icon left + inline header row "ACCOUNT STATUS — GOOD STANDING" (GOOD STANDING in green `#1FB358`) + warning paragraph in secondary text.
    - Account Progress (purple-bordered): bar-chart icon + "ACCOUNT PROGRESS" header + 5 red progress bars (Achievements Unlocked, Permanent Buffs, Heroes, Companions, Challenges) with monospaced count labels right-aligned.
  - RIGHT (outer purple container wrapping):
...
- No description band.
- Main content outer purple container with two stacked category rows. Each row: left vertical category label panel (icon + multi-line category name in purple accent) + 4 drug cards horizontally.
...
- Status notification panel (thin, purple-bordered): "Community catalog refreshed (3 entries)." in purple accent.
- Two-column body:
...
- Three stacked settings panels:
  - RETRO FX MASTER ENABLE (purple-bordered): left side header + description + status note; right side ON (Selected) / OFF (Default) buttons row + APPLY button (Selected, no arrow flourishes).
...
- Filter row: Solo dropdown (Selected red, left) + "Showing Solo saves stored on this machine." status text (purple accent, middle) + "Page 1 / 1" indicator (right).
- 2×2 grid of save slot panels (each purple-bordered):
  - Header "SAVE SLOT X" in purple accent.
  - Two-column body: 4 character portrait slots (silhouettes + role icons) on left + 5 metadata rows (Date / Difficulty / Stage / Score / Time, labels purple accent, values white) on right.
...
- This master plan: `C:\UE\T66\UI\T66_UI_FLAT_REDESIGN_PLAN.md`
