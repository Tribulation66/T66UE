# Reference Generation Notes

- Chat number: 2
- Screen slug: `player_summary_picker`
- Source current screenshot: `C:\UE\T66\UI\screens\player_summary_picker\current\current_state_1920x1080.png`
- Anchor path: `C:\UE\T66\UI\screens\main_menu\reference\main_menu_reference_1920x1080.png`
- Source screenshot state: existed before generation
- Raw native image output: `C:\Users\DoPra\.codex\generated_images\019dc448-af5d-72b1-8abf-524ea1622a8b\ig_0de50fc64c2798b60169eca6c9d9508198aed722da3a6a1366.png`
- Normalization: deterministic resample from `1672x941` to `1920x1080`
- Accepted reference: `C:\UE\T66\UI\screens\player_summary_picker\reference\player_summary_picker_reference_1920x1080.png`
- Status: accepted for first reference-generation pass

## Runtime-Owned Regions To Preserve Later

- title text
- empty-state text
- future player rows, avatars, names, and selected state
- dimmed main-menu background live UI

## Integrator Notes

- Empty-state content remains runtime-owned.
- Full-screen reference is an offline comparison target only.
