...
DEST_ROOT = "/Game/Characters/Enemies/Regular"
...
def _source_glb(run_root, enemy_id):
    return os.path.join(run_root, enemy_id, "Models", f"{enemy_id}_QuadRetro.glb").replace("\\", "/")


def _dest_dir(enemy_id):
    return f"{DEST_ROOT}/{enemy_id}/QuadRetro"


def _asset_name(enemy_id):
    return f"SM_{enemy_id}_QuadRetro"
...
def _load_enemy_rows(csv_path):
    ...
    if len(rows) != 25:
        raise RuntimeError(f"Expected 25 enemy rows, found {len(rows)} at {csv_path}")
...
def _bind_imported_texture_to_materials(dest_dir, dest_name, source_glb):
    ...
    if not texture:
        unreal.log_warning(f"[QuadRetroEnemies] No imported pixelated texture found for {dest_name}")
        return False

    materials = ImportStaticMeshes._find_candidate_materials(dest_dir, dest_name)
    if not materials:
        unreal.log_warning(f"[QuadRetroEnemies] No materials found for {dest_name}")
        return False
    ...
    unreal.log(f"[QuadRetroEnemies] Bound {dest_name} materials to {texture.get_path_name()}")
    return True
...
def _import_one(enemy_id, display_name, source_glb):
...
def _mark_enemy_rows_mesh_ready(csv_path, import_reports):
    ...
    missing_status_column = "ModelStatus" not in fieldnames
    if missing_status_column:
...
def main():
    ...
    enemies_csv = os.path.join(project_dir, ENEMIES_CSV_RELATIVE).replace("\\", "/")
    visuals_csv = os.path.join(project_dir, CHARACTER_VISUALS_CSV_RELATIVE).replace("\\", "/")
    ...
    for row in enemy_rows:
        ...
        source_glb = _source_glb(run_root, enemy_id)
        try:
            report = _import_one(enemy_id, display_name, source_glb)
            import_reports.append(report)
            unreal.log(f"[QuadRetroEnemies] Imported {enemy_id}")
        except Exception as exc:
            failures.append({"enemy_id": enemy_id, "error": str(exc)})
    ...
    if failures:
        ...
        raise RuntimeError(f"Quad Retro enemy import failed for {len(failures)} asset(s); see {report_path}")
    ...
    unreal.log(f"[QuadRetroEnemies] Imported and registered {len(import_reports)} enemy visuals")
    unreal.log(f"[QuadRetroEnemies] Wrote {report_path}")
