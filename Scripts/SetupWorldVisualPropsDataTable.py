"""
Create or reload DT_WorldVisualProps from Content/Data/WorldVisualProps.json.

Run from the editor or command line:
  UnrealEditor-Cmd.exe T66.uproject -run=pythonscript -script="C:/UE/T66/Scripts/SetupWorldVisualPropsDataTable.py"
"""
...
DT_PATH = "/Game/Data/DT_WorldVisualProps"


def resolve_row_struct():
    struct_type = getattr(unreal, "T66WorldVisualPropRow", None)
    if struct_type is None:
        unreal.log_error("Could not resolve T66WorldVisualPropRow in Python.")
        return None
...
def main():
    unreal.log("=== SetupWorldVisualPropsDataTable ===")
    ...
    json_path = os.path.normpath(os.path.join(project_dir, "Content", "Data", "WorldVisualProps.json"))
    if not os.path.exists(json_path):
        unreal.log_error(f"WorldVisualProps.json not found at {json_path}")
        return
    ...
    if not success:
        unreal.log_error("Failed to fill DT_WorldVisualProps from JSON.")
        return
    ...
    unreal.log(f"DT_WorldVisualProps reloaded from {json_path}")
    unreal.log("=== SetupWorldVisualPropsDataTable DONE ===")
