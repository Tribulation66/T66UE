"""
Create or reload DT_VehicleInteractables from Content/Data/VehicleInteractables.json.

Run from the editor or command line:
  UnrealEditor-Cmd.exe T66.uproject -run=pythonscript -script="C:/UE/T66/Scripts/SetupVehicleInteractablesDataTable.py"
"""
...
DT_PATH = "/Game/Data/DT_VehicleInteractables"


def resolve_row_struct():
    struct_type = getattr(unreal, "T66VehicleInteractableRow", None)
    if struct_type is None:
        unreal.log_error("Could not resolve T66VehicleInteractableRow in Python.")
        return None
...
def main():
    unreal.log("=== SetupVehicleInteractablesDataTable ===")
    ...
    json_path = os.path.normpath(os.path.join(project_dir, "Content", "Data", "VehicleInteractables.json"))
    if not os.path.exists(json_path):
        unreal.log_error(f"VehicleInteractables.json not found at {json_path}")
        return
    ...
    if not success:
        unreal.log_error("Failed to fill DT_VehicleInteractables from JSON.")
        return
    ...
    unreal.log(f"DT_VehicleInteractables reloaded from {json_path}")
    unreal.log("=== SetupVehicleInteractablesDataTable DONE ===")
