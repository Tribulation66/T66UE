"""
Create or reload DT_DifficultyTuning from Content/Data/DifficultyTuning.json.

Run from the editor or command line:
  UnrealEditor-Cmd.exe T66.uproject -run=pythonscript -script="C:/UE/T66/Scripts/SetupDifficultyTuningDataTable.py"
"""
...
DT_PATH = "/Game/Data/DT_DifficultyTuning"


def resolve_row_struct():
    struct_type = getattr(unreal, "T66DifficultyTuningRow", None)
    if struct_type is None:
        unreal.log_error("Could not resolve T66DifficultyTuningRow in Python.")
        return None
...
def main():
    unreal.log("=== SetupDifficultyTuningDataTable ===")
    ...
    json_path = os.path.normpath(os.path.join(project_dir, "Content", "Data", "DifficultyTuning.json"))
    if not os.path.exists(json_path):
        unreal.log_error(f"DifficultyTuning.json not found at {json_path}")
        return
    ...
    if not success:
        unreal.log_error("Failed to fill DT_DifficultyTuning from JSON.")
        return
    ...
    unreal.log(f"DT_DifficultyTuning reloaded from {json_path}")
    unreal.log("=== SetupDifficultyTuningDataTable DONE ===")
