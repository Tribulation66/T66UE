"""
Import generated panel texture PNGs into Unreal as UTexture2D assets.

Source: SourceAssets/Images/Panels/PanelDark.png (etc.)
Dest:   /Game/UI/Assets/PanelDark  (etc.)

Run in Unreal Editor: Tools -> Execute Python Script -> Scripts/ImportPanelTextures.py
"""
...
def main():
    unreal.log("=== Import Panel Textures ===")
    proj = _project_dir()
    src_dir = os.path.join(proj, "SourceAssets", "Images", "Panels")
    ...
    if not tasks:
        unreal.log_error(f"{LOG_PREFIX} No textures found. Run GeneratePanelTextures.py first.")
        return
    ...
    unreal.log("=== Import Panel Textures DONE ===")
