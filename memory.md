...
- **Heros/Heroes path:** Fixed `T66MusicSubsystem.cpp` hero music folder to `/Game/Audio/OSTS/Heroes/`. `Content/Characters/Heros/` renamed to `Heroes/` in UE Editor; CSV paths and import scripts updated. `Content/UI/Sprites/Heros/` still uses old name (rename separately in UE Editor when ready).
...
- **Heros → Heroes folder rename (mostly done):** `Content/Characters/Heros/` renamed to `Heroes/` in UE Editor, redirectors fixed, `CharacterVisuals.csv` paths updated, `ImportHeros.py` and `DiagnoseMaterials.py` updated. **Remaining:** `Content/UI/Sprites/Heros/` still uses old name — rename in UE Editor, fix redirectors, then update `Heroes.csv` portrait paths and `ImportSpriteTextures.py`/`ImportHeros.py` sprite paths. Also consider renaming `SourceAssets/Heros/` and the script file itself (`ImportHeros.py` → `ImportHeroes.py`).
