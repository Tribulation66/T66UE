...
- **Heros/Heroes path:** Fixed `T66MusicSubsystem.cpp` hero music folder to `/Game/Audio/OSTS/Heroes/` to match the on-disk folder. The plan is to rename `Content/Characters/Heros/` → `Heroes/` in the UE Editor (creates redirectors), then fix up all CSV paths and redirectors. See tech debt section.
...
- **Heros → Heroes folder rename (in progress):** Music subsystem code path fixed to `/Game/Audio/OSTS/Heroes/`. Unused `Content/Characters/Heroes/` (legacy Meshy exports) deleted. Remaining: rename `Content/Characters/Heros/` → `Heroes/` **in the UE Editor** (Content Browser rename → Fix Up Redirectors), then update `CharacterVisuals.csv` paths and any import scripts. Do in a dedicated session with checkpoint commit.
