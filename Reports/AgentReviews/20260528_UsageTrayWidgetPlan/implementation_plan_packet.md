# AI Usage And Operator Tray Widget Implementation Plan Packet

## Working Goal

Create a full reviewed implementation plan for a Windows Claude/Codex usage tray widget showing the active Operator plus each provider logo with weekly remaining percent, without building or executing it yet.

## User Request

The user wants a small Windows taskbar-area widget that shows the current Operator plus the Claude and Codex logos with the weekly remaining percentage next to each logo. They explicitly said not to build or execute it yet, and asked for the full plan to execute it.

## Clarified Goal

Plan a local Windows utility that displays the active Operator/Validator state plus two compact provider rows/icons:
...
- Operator display should be driven by a local state file once implemented. No durable operator-state file exists yet, so the first implementation must handle missing state as `Operator: Unknown` rather than guessing.
...
- `AGENTS.md`
  ...
  - Validator review required before Tier 1 substantive output.
  - Current role language should be Operator/Validator first, with Claude/Codex as concrete model assignments.
...
- No current operator-state source file exists yet. Operator state is currently conversation/task-scoped process state, not machine-readable widget state.
...
```text
PPF CHECK
Objective: Build a small Windows taskbar-area usage widget showing the active Operator plus Claude and Codex logos with weekly remaining percentages.
...
My planned implementation: A native .NET Windows Desktop/WPF app with a tray icon, taskbar-adjacent always-on-top compact window, structured collectors for Claude and Codex usage, an operator-state reader, no OCR by default, and safe credential handling.
...
- `C:\Users\DoPra\Tools\AIUsageTray\src\T66.AIUsageTray\Models\OperatorState.cs`
...
- `Tools/UsageTray/src/T66.AIUsageTray/Models/OperatorState.cs`
- `Tools/UsageTray/src/T66.AIUsageTray/Services/PollingUsageService.cs`
- `Tools/UsageTray/src/T66.AIUsageTray/Services/OperatorStateService.cs`
...
- Operator state source and last update time
...
OperatorState
  ...
  Scope: Global | Thread | Manual | Unknown
  Source: StateFile | ManualTraySelection | FutureAgentHelper | Unknown
  UpdatedAtLocal: DateTimeOffset?
  IsStale: bool
```
...
1. Read `%LOCALAPPDATA%\T66UsageTray\operator-state.json` if present.
2. Parse `Operator`, `Validator`, `Scope`, `Source`, and `UpdatedAtLocal`.
3. Treat missing or malformed state as `Operator: Unknown`.
4. Display stale state distinctly if `UpdatedAtLocal` is older than the configured threshold.
5. Allow a manual tray menu selection as the first implementation path:
   - Set Operator = Claude, Validator = Codex.
   - Set Operator = Codex, Validator = Claude.
   - Set Operator = Unknown.
6. Future process integration can update the same file after `Make Claude operator` or `Make Codex operator`; that helper is not built in this first widget plan.
7. Never store provider tokens or raw provider JSON in the operator-state file.
...
- `operator-state.json` stores only Operator/Validator role metadata and timestamps.
...
1. Unit tests:
   ...
   - Operator-state fixture parses Operator, Validator, Scope, Source, and timestamp.
   - Missing or malformed operator state shows `Unknown` without crashing.
...
3. UI smoke:
   ...
   - Confirm compact window shows `Operator:` and both provider usage values.
   - Confirm compact window shows both logos and percentages.
   - Confirm manual Operator switch updates the visible `Operator:` line.
...
- Updating operator state automatically from Codex/Claude chat commands in the first implementation. The first implementation may provide manual tray selection and a documented state-file format; automatic agent writes can be a later reviewed helper.
...
- Stale operator display. Mitigation: include source/timestamp, show stale/unknown state clearly, and do not guess when no state exists.
...
7. Implement `OperatorStateService` with missing-state fallback and manual tray selection.
8. Implement WPF compact widget UI and tray context menu, including the `Operator:` line.
9. Add settings/cache/logging with redaction.
10. Add publish script or README commands.
11. Run tests and live read-only probes.
12. Launch UI manually and screenshot/inspect the result.
13. Optional: ask fresh approval, then add startup shortcut if approved.
...
- Whether the added Operator/Validator state display is correctly scoped as plan-only and does not imply a durable state helper already exists.
