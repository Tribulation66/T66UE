...
- Weekly display should mean remaining percentage after provider-specific normalization. Codex `usedPercent` is already a 0-100 percent. Claude `utilization` must be detected and normalized before display; if it is 0-1, multiply by 100 first, and if it is already 0-100, use as-is. Then calculate `100 - normalizedUsedPercent`.
...
5. Normalize `utilization` defensively:
   - If value is between `0` and `1`, treat it as a fraction and multiply by `100`.
   - If value is between `1` and `100`, treat it as an already-scaled percent.
   - Clamp normalized used percent to `0-100` before calculating remaining.
...
8. Treat Claude Code statusline ingestion as a later optimization, not a v1 prerequisite.
...
`Saved/` is ignored by `.gitignore`, so this publish target keeps build output out of the durable source tree.
...
4. Implement Claude collector with the proven OAuth usage read as v1; leave statusline ingestion for a later improvement.
