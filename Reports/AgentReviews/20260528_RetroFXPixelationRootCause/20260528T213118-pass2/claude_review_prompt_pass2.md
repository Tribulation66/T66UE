...
Review scope:
- Packet path: C:\UE\T66\Reports\AgentReviews\20260528_RetroFXPixelationRootCause\root_cause_report.md
...
- Runtime `r.ScreenPercentage` restored to the project/user native path, normally 100 from `ApplyCrispRenderingDefaults()` and config unless another graphics setting intentionally changes it.
...
6. Add a diagnostic smoke check for the patch.
   - Fresh/no-save launch: log or assert effective gameplay RetroFX off, real low-resolution off, `r.ScreenPercentage` native/100 path.
