...
Use option 2 as the primary implementation and preserve option 3 as a fallback.
...
1. Add a `TaskbarHostService` in the tray app that:
   ...
   - Exposes a fallback positioning path if the taskbar host cannot be found or docking fails.
...
3. Update `MainWindow.xaml.cs`:
   ...
   - Fall back to the old floating position if docking fails.
...
- Risk: Windows 11 taskbar internals may reject or hide a child WPF window.
  - Mitigation: explicit fallback to the prior floating overlay behavior.
...
- Parent-window or window-rect evidence showing taskbar docking attempt/result.
