...
1. Mechanism: weapon impact context creation
   Required: YES
   Planned implementation: Add a small `FT66CombatImpactContext` in `T66CombatComponent.h` carrying source type, source ID, hero ID, attack category, attack origin, impact point, forward vector, primary target handle, hit target handles, effective damage, and optional parent/chain data. Shape fields are category-tagged: AOE populates radius/inner radius/sector metadata; Pierce populates line length/tube radius; Bounce populates hop/chain index; DOT populates anchor/follow target only. Fill one or more contexts from Pierce, AOE, Bounce, and DOT paths without changing weapon damage behavior. For non-AOE weapon categories, the context impact point is populated as the most recent meaningful combat point for diagnostics/future chaining only; this pass does not consume those non-AOE points for new gameplay.
...
3. Mechanism: idol-owned damage source
   Required: YES
   Planned implementation: Keep `SourceID=IdolID` when applying idol damage. For `Idol_Water` only, replace the old `WeaponHitActors` target source with target handles gathered by a Water-owned AOE sphere centered at the idol impact point. The Water query radius is `FIdolData::AoeRadius` from `Idol_Water` (`300` in the current CSV), not the weapon slash radius; if `AoeRadius <= 0`, fallback to `GetPropertyAtRarity`. Do not apply weapon AOE radius, weapon inner radius, or item/hero category scaling to this first structural Water query. The current live idol damage path does not consume `AoeDelay`; this pass preserves that immediate timing rather than adding a timing change. Log `AoeDelay=0.15 Applied=false Reason=LegacyImmediatePreserved` so the deferred timing gap is explicit. For `Idol_Earth`, `Idol_Storm`, and all non-AOE idols, this pass must be gameplay-observable neutral: keep the current target/damage behavior and add only context/logging plumbing unless a later reviewed proof expands their behavior.
   Evidence needed: proof log/damage totals showing `Idol_Water` separately from `AutoAttack`, `RadiusSource=IdolData.AoeRadius Radius=300`, `AoeDelay=0.15 Applied=false`, and HP deltas on expected Water-hit targets.

4. Mechanism: temporary Water placeholder at impact
   Required: YES for structural proof
   Planned implementation: Add a helper that spawns a short-lived, collisionless, blue static-mesh sphere at the Water idol impact point when no production `IdolModifier` binding exists. The mesh source is the engine primitive sphere already exposed through `FT66VisualUtil::GetBasicShapeSphere` in `Source/T66/Gameplay/T66VisualUtil.h`; no new asset is created. The placeholder spawns at the same immediate time as Water damage application in this structural pass. It must log `CombatVFXIdolImpactPlaceholderSpawned` and not write production assets.
...
5. Mechanism: production binding future seam
   ...
   Planned implementation: Add `TrySpawnBoundIdolImpactVFX` or equivalent helper using existing `ResolveCombatVFXBinding(IdolModifier, IdolID, IdolCategory, ...)`. If a real row exists later, it should spawn the production Niagara at the idol impact context and suppress the temporary placeholder for that idol. `ImpactPresentationActive` means either this future bound `IdolModifier` Niagara path or the temporary Water placeholder path is active, so the old projectile lane stays mutually exclusive in both cases.
...
6. Mechanism: proof-mode equipment and capture
   Required: YES
   Planned implementation: Add a non-shipping proof mode such as `hero1axeaoewateridolimpact` that snapshots current equipped idols/tier values, equips Hero 1 black AOE, temporarily equips a requested proof idol, stages fixed targets around the slash impact point, fires one real combat attack, then logs weapon damage, idol damage, placeholder/presentation state, and target pass/fail results. The default proof idol is `Idol_Water`. A secondary neutral proof run will use `Idol_Earth` to prove non-Water AOE idols stay on the legacy path. After proof logging, restore the previous idol state via the idol manager so an editor session is clean even though the normal capture process exits. Extend `CaptureT66GameplayVideo.ps1` and likely add a wrapper script for this mode.
...
- `Source/T66/Gameplay/T66CombatComponent.cpp`
  ...
  - Use `Idol_Water.AoeRadius` as the Water query radius and preserve legacy immediate timing by not applying `AoeDelay` yet.
...
- `Source/T66/Gameplay/T66CombatVFX.cpp`
  - Add bound idol impact VFX and temporary Water sphere placeholder helper implementation. Include or reuse `Gameplay/T66VisualUtil.h` and `Components/StaticMeshComponent.h` as needed for the engine-sphere placeholder.
...
- `Gameplay/Combat/MASTER_COMBAT.md`
  - Document the new impact context structure, Water placeholder proof boundary, final Niagara deferral, and architecture reconciliation: idol overlays are additive presentation, but idols may also own gameplay damage/query authority when their `FIdolData` category says they are independent attack sources.
...
- Applying `FIdolData::AoeDelay` timing semantics. Current live idol damage is immediate; preserving that behavior is intentional for this structural proof. If the implementation confirms no existing runtime consumer of `AoeDelay`, record a pending issue for future idol timing semantics instead of silently changing timing now.
...
- Risk: changing idol damage selection could alter normal gameplay unexpectedly.
  - Mitigation: use context-driven target selection only for `Idol_Water`; all other idols are behavior-neutral in this pass. The proof mode includes Water-hit and Water-miss targets plus `DamageBySource` separation, and a secondary `Idol_Earth` neutral proof confirms the old AOE idol path still runs without impact-presentation suppression.
...
- Risk: `AoeDelay` and radius semantics are ambiguous.
  - Mitigation: first pass uses direct `IdolData.AoeRadius` and preserves legacy immediate timing. Proof logs must report `RadiusSource=IdolData.AoeRadius`, `Radius=300`, `AoeDelay=0.15`, and `Applied=false`.
...
4. Inspect/copy proof log excerpt showing:
   ...
   - `RadiusSource=IdolData.AoeRadius Radius=300`.
   - `AoeDelay=0.15 Applied=false Reason=LegacyImmediatePreserved`.
   - `DamageBySource` or equivalent log with `Idol_Water` > 0.
   - Target HP pass/fail results for Water-hit and Water-miss targets.
5. Run the neutral proof configuration with `Idol_Earth` and confirm:
   - `Idol_Earth` uses the legacy idol target path.
   - No `CombatVFXIdolProjectileLaneSuppressed SourceID=Idol_Earth` line appears.
   - `DamageBySource` includes `Idol_Earth` damage through the legacy path.
6. Add a source-gate snippet to the proof summary showing the new Water target-query branch is guarded by `IdolID == Idol_Water`, while `Idol_Earth`, `Idol_Storm`, and non-AOE idols still route through the legacy behavior block.
7. Confirm `Content/Data/Idols.csv`, `Content/Data/CombatVFXBindings.csv`, and `Content/Data/DT_CombatVFXBindings.uasset` are unchanged by this structure pass.
8. Refresh staged standalone with `Scripts/StageStandaloneBuild.ps1` and verify the taskbar shortcut target if the runtime gameplay change is accepted into the playable build.
...
## Reconciliation From Claude Pass 2

Claude pass 2 returned `Verdict: REVISE` with three Major issues:

1. The Water `AoeDelay=0.15` field was not addressed.
2. The Water target-query radius source was not specified.
3. Behavior-neutral idols needed regression evidence.

Accepted. This revision states:

- The existing live idol damage path does not consume `AoeDelay`; this structural pass preserves immediate damage/placeholder timing and logs `AoeDelay=0.15 Applied=false Reason=LegacyImmediatePreserved`.
- The Water query radius is `IdolData.AoeRadius` (`300` from the current CSV), not the weapon slash radius or item-scaled radius.
- The proof includes a Water run plus an `Idol_Earth` neutral run and source-gate evidence that only `Idol_Water` enters the new target-query branch.
- Non-AOE weapon impact points are diagnostic/future-chain values only in this pass; no current consumer is added for them.
- `MASTER_COMBAT.md` must reconcile the architecture doc wording by separating idol additive presentation from idol-owned gameplay damage/query authority.
