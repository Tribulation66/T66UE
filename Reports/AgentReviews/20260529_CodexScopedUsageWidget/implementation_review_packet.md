...
2. Add a Codex window tracker service:
   ...
   - Drive visibility and placement with a 500ms `DispatcherTimer`; this avoids modifying Codex internals while keeping the overlay responsive to focus, move, and resize changes.
...
- Move/resize-follow check proving the overlay re-anchors after the Codex window rect changes.

## Rollback

The utility source folder is local editable source under `C:\Users\DoPra\Tools\AIUsageTray`. Rollback is to remove the Codex window tracker/overlay positioning edits and restore the previous floating overlay implementation, then republish `%LOCALAPPDATA%\T66UsageTray\app`. The previous taskbar attempt files are not production dependencies and should be deleted or left unused only if rollback/debug context is needed.
