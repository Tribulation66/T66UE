...
**2026-06-09 — Phase 3b `+%` display language landed** (`T66StatsPanelSlate.{h,cpp}`, `T66PowerUpScreen.cpp`, + one declaration move in `T66RunStateSubsystem.h`):
- **Unified stat rows** (stats panel, all three variants: static, live, snapshot): now `Label: +X%` where X = accumulated bonus points (`FMath::RoundToInt`, clamped >= 0; 1 point == 1%). Live reads `RunState->GetStatBonusValue()`; chance-class stats render identically (points are percentage points per A1). New key `T66.StatsPanel/StatBonusPercent` = `+{0}%`.
- **Derived rows** (Total Damage Reduction / Total Dodge Chance): now `Label: N%` (`CeilToInt(Value*100)`), computation untouched. New key `EffectivePercent` = `{0}%`.
- **Primary purge (§1.6):** the 8 primary-stat lines removed from all three panel variants (the index-0 Level line was never rendered — update loops always started at 1 — so nothing needed keeping); category headers are label-only (`Damage`, not `Damage: 57/99`); the adjective ladder (Mickle…Poxy) and `ValueOutOf99`/`CategoryHeaderWithValue`/`BaseStatLineWithAdjective` keys retired; `FT66LiveStatsPanel.PrimaryLines` member deleted (no external users).
- **PowerUp relics:** `RelicSecondaryEffectFormat` and `RelicPrimaryEffectFormat` both changed to `+{0}% {1}` (same keys; base-axis relics also apply percent via `T66MapBaseStatToUnifiedStat`, so both branches earn the `%`). `DisplayBonus = BonusStatPoints × tier` verified equal to the percent actually applied (`GetRelicStatBonus`/`GetRelicBaseStatBonus` → `WholeStatToTenths` → bonus points). `GetSingleUseEffectText` untouched (already `+X%`).
- **Access note:** `GetStatBonusValue` was private; its declaration was moved to the public section of `T66RunStateSubsystem.h` (declaration relocation only, zero behavior change) so display code can read bonus points.
- **Snapshot-bonus decision:** `Snapshot->StatValues` is now *read* as accumulated bonus points (consistent with live and with the `stat_bonuses` wire key). The two writers still store other units and were NOT touched (outside Phase 3b file scope) — follow-ups: `T66LeaderboardSubsystem.cpp` end-of-run snapshot stores `GetStatValue` (effective) → switch to `GetStatBonusValue`; `T66HeroSelectionScreen_Stats.cpp::PopulateHeroStatsSnapshot` stores raw hero bases → store zeros (a fresh hero has no accumulated bonuses) or stop populating. Until then the hero-select full-stats overlay (the only in-repo consumer of the snapshot panel) shows base-values-as-bonus numbers; `T66RunSummaryScreen.cpp` also reads `StatValues` directly with effective-value assumptions (its own rows; out of scope here).
- **Save-field review:** legacy base-stat snapshot fields the stats panel still reads: `Snapshot->ArmorStat` and `Snapshot->EvasionStat` (inputs to `GetDerivedStatValueFromSnapshot` for the derived Armor Reduction / Dodge Chance rows) — retained deliberately as derived-display inputs. `DamageStat/AttackSpeedStat/AttackScaleStat/AccuracyStat/LuckStat/SpeedStat` and `HeroLevel` are no longer read by the stats panel (still serialized; other screens may read them).
- Panel category arrays untouched (per scope): the new umbrella/utility stats (All Damage / All Attack Speed / All Scale / Accuracy / Luck / Move Speed / HP Regen) are not yet rows in the panel — candidate Phase 3c content change.
- **2026-06-09 — snapshot writers aligned:** leaderboard end-of-run snapshot now stores `GetStatBonusValue` (bonus points, `T66LeaderboardSubsystem.cpp`); hero-select populator no longer fills `StatValues` (kept `Reset()` only — fresh run has zero accumulated bonus; panel renders missing entries as `+0%` and hero-select passes `bExtended=true` explicitly); `T66RunSummaryScreen.cpp` rendered saved `StatValues` as raw floats in its own rows (saved elemental-power lines + saved all-stats loop) → now `+X%` via new `T66.RunSummary/StatBonusPercent` key, same rounding as the panel (live `RunState->GetStatValue` rows untouched — not snapshot data).

**Remaining (presentation + balance, deferred):** `+%` language for tooltips / item cards (`T66TooltipResolvers`, `T66ItemCardTextUtils`), surfacing the umbrella/utility stats in the panel categories, and balance playtest tuning of the routed `%` magnitudes.
