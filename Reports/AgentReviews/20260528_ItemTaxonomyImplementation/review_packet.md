...
- `Content/Data/Items.csv` has exactly these columns: row name, `ItemID`, `Icon`, `BlackIcon`, `RedIcon`, `YellowIcon`, `WhiteIcon`, `PrimaryStatType`, `SecondaryStatType`, `BaseBuyGold`, `BaseSellGold`. There are no description/flavor/rarity-weight columns in this CSV.
- `T66RunStateSubsystem_Combat.cpp` confirms Counter and Reflect already damage bosses through non-OHKO half-damage paths (`CounterDmg` / `ReflectedAmount`). Assassinate and Crush are the only reviewed defensive/evasion paths still using lethal `99999` boss damage.
- Minibosses are enemy/mob promotions, not `AT66BossBase` subclasses: `AT66GameMode::EnsurePlacedTowerMinibossForFloor()` returns `AT66EnemyBase*`, `AT66EnemyBase` owns `bool bIsMiniBoss` and `ApplyMiniBossMultipliers()`, `AT66EnemyDirector` stores `TWeakObjectPtr<AT66EnemyBase> ActiveMiniBoss`, and `AT66MobBase` also owns `bool bIsMiniBoss`. `AT66BossBase` derives from `ACharacter` and is used for true bosses/gambler boss.
...
- Retired item IDs other than the token alias are not normalized to replacement items. Save/inventory validation should warn-and-skip/remove stale slots for `Item_Accuracy`, `Item_Cheating`, `Item_Stealing`, `Item_HpRegen`, and `Item_LifeSteal` rather than synthesize rows or reintroduce them to the live DataTable.
...
1. Data and ordering
   ...
   - Exact new/reused row schema:
     - `Item_Execute`: reuse `Item_Accuracy_*` icon paths, `PrimaryStatType=Accuracy`, `SecondaryStatType=Execute`, `BaseBuyGold=55`, `BaseSellGold=27`.
     - `Item_LootBag`: reuse `Item_LootCrate_*` icon paths, `PrimaryStatType=Luck`, `SecondaryStatType=LootBag`, `BaseBuyGold=45`, `BaseSellGold=22`.
     - `Item_LootWheel`: reuse `Item_TreasureChest_*` icon paths, `PrimaryStatType=Luck`, `SecondaryStatType=LootWheel`, `BaseBuyGold=45`, `BaseSellGold=22`.
     - `Item_VendorToken`: reuse `Item_BackroomsQuickRevive` icon path for all icon columns until dedicated art exists, `PrimaryStatType=Special`, `SecondaryStatType=VendorToken`, `BaseBuyGold=100`, `BaseSellGold=0`. These buy/sell values match the current synthetic `Item_GamblersToken` fallback values.
   - Ensure no `Item_GamblersToken` row exists in `Items.csv`; the legacy ID survives only through `NormalizeLegacyItemID()` and token helper aliases.
...
2. Schema and stat families
   ...
   - Mark old live-facing `Accuracy`, `Cheating`, `Stealing`, and `GamblerToken` as deprecated by adding them to `T66IsDeprecatedSecondaryStatType()`. Do not add `UMETA(Hidden)` this pass; the reason is editor-picker preservation for legacy/diagnostic authoring while runtime-live surfaces are controlled by the existing live filter. Keep internal hero head-targeting `Accuracy` math available for `GetAccuracyChance01()`.
   ...
   - Backend parsers keep accepting old payload keys (`Accuracy`, `Cheating`, `Stealing`, `GamblerToken` if present) for historical run-summary compatibility, but serializer/live snapshots should only emit live keys plus compatibility keys `EvasionChance` and `TreasureChest`.
   ...
   - Do not broadly rewrite `GSingleUseBuffStats`; its callers already use live-stat filtering. Add defensive names/slugs/icons/order updates in these named switch/list surfaces:
     - `UT66BuffSubsystem` index/live-filter behavior only where new enum values need array/index support.
     - `T66BackendRunSerializer` and `T66BackendRunSummaryParser` secondary-stat payload key switch/parser.
     - `T66TemporaryBuffUIUtils::GetTemporaryBuffIconSlug()`.
     - `T66PowerUpScreen` shop slug, drug name/effect text, and grouped stat display lists.
     - `T66HeroSelectionScreen_Private` drug-name helper and `T66HeroSelectionScreen_Stats` baseline values for new live secondaries.
     - `T66StatsPanelSlate` category arrays / family checks.
     - `T66LocalizationSubsystem` display name, tooltip, and item base-name switches.
   ...
   - Confirm the retained hero head-targeting path does not rely on `T66IsAccuracyFamilySecondaryStatType()` membership for old `Accuracy`: `GetAccuracyChance01()` reads `GetSecondaryStatValue(Accuracy)` directly, and hero selection seeds `Accuracy` directly from `HeroData.BaseAccuracy`.
   - Repo-wide grep target is explicit: `T66IsAccuracyFamilySecondaryStatType`, `ET66SecondaryStatType::Accuracy`, and `static_cast<.*ET66SecondaryStatType` upper-bound/index arrays. Current family helper callsites are only in `T66DataTypes.h` through `T66ResolveEffectivePrimaryStatType`; UI/static arrays in `T66StatsPanelSlate` and `T66PowerUpScreen` are separate explicit lists and will be updated.
...
4. Central OHKO rule
   ...
   - Helper signature will take `AActor* TargetActor`, an optional `const FT66CombatTargetHandle* TargetHandle`, `FName DamageSourceID`, and `FName EventType`.
   - Exact default-handle contract:
     - `AT66BossBase`: no-op and return false before resolving a handle.
     - `AT66EnemyBase`: use the supplied valid handle, else `Enemy->ResolveCombatTargetHandle(nullptr, ET66HitZoneType::Body)`, then call `TakeDamageFromHeroHitZone(MaxHP + CurrentHP + 1, Handle, DamageSourceID, EventType)`. If handle resolution is unexpectedly invalid, fall back to `TakeDamageFromHero(MaxHP + CurrentHP + 1, DamageSourceID, EventType)`.
     - `AT66MobBase`: use the supplied valid handle, else `Mob->ResolveCombatTargetHandle(nullptr, ET66HitZoneType::Body)`, then call `TakeDamageFromHeroHitZone(MaxHP + CurrentHP + 1, Handle, DamageSourceID, EventType)`; if the mob is already dying/dead or the handle is invalid, return false.
...
5. Reward source wiring
   ...
   - Shared discrete rarity formula for Loot Bag and Loot Wheel item rewards:
     - Clamp multiplier to `1.0..3.0`.
     - `TierAdvance = floor(Multiplier - 1.0)`.
     - Apply `TierAdvance` to item rarity, clamped at White. Examples: `1.0 => no tier change`, `2.0 => +1 tier`, `3.0 => +2 tiers`.
     - Intermediate values intentionally use breakpoint behavior (`1.0..1.99 => no tier change`, `2.0..2.99 => +1 tier`) to keep single-reward outcomes deterministic and seed-independent.
     - This is a deterministic single-reward analogue to Loot Crate's multiplier bias: Crate shifts a weighted strip before the roll; Bag/Wheel upgrade the already-locked single reward because there is no strip/reward-weight surface at consumption time.
   - Loot Chest: apply `GetLootChestRewardMultiplier()` in `AT66ChestInteractable::LockChestReward()` to the rolled chest gold result after the normal range roll, rounded to int and clamped non-negative. Do not scale both range and result, and do not rename the compatible `TreasureChest` data key. Examples: `1.0 => Gold`, `2.0 => Gold * 2`.
   - Loot Bag: apply `GetLootBagRewardMultiplier()` only when the loot bag interactable is consumed in `AT66PlayerController::HandleSecondaryInteract()`, using the shared discrete rarity formula before `AddItemWithRarity()` and before the reveal HUD. Do not reroll weights, and do not patch enemy loot bag drop counts, world spawn count, or arcade reward spawning in this pass.
   - Loot Wheel: apply `GetLootWheelRewardMultiplier()` only inside `AT66LootWheelInteractable::LockLootWheelReward()` after reward type selection:
     - Gold reward: multiply the rolled gold result by the multiplier, rounded to int and clamped non-negative.
     - Item reward: use the shared discrete rarity formula after item selection.
     - Boost reward: multiply only `BoostBonusStatPoints` by the multiplier with `FMath::RoundToInt`, clamp to at least 1, and keep `BoostDurationSeconds` at the existing 10 seconds to avoid compounding value across points and duration. Examples: `1.0 => 8 points / 10s`, `2.0 => 16 points / 10s`, `3.0 => 24 points / 10s`.
     - Do not reroll reward type weights, and do not patch world loot wheel spawn counts or gallery/showcase spawns.
...
6. Vendor Token canonicalization
   ...
   - Add compatibility comments at retained internal schema/function names (`ActiveGamblersTokenLevel`, `GamblersTokenUnlockedLevel`, `ApplyGamblersTokenPickup()` and close helpers) stating that the old name is retained for save/API compatibility and the canonical player-facing item is Vendor Token.
...
8. Data asset reload and verification
   ...
   - Run Unreal reload for `DT_Items` with:
     - `& "C:\Program Files\Epic Games\UE_5.7\Engine\Binaries\Win64\UnrealEditor-Cmd.exe" "C:\UE\T66\T66.uproject" -run=pythonscript -script="C:\UE\T66\Scripts\SetupItemsDataTable.py" -unattended -nop4 -nosplash`
   ...
   - Add a narrow, non-shipping command-line smoke path, because no existing hook directly proves all changed seams. New verification infrastructure is explicitly in scope for this pass because Claude required runtime proof of the user-clarified OHKO rule and reward multiplier behavior. The planned hook is `-T66ItemTaxonomySmoke=<OutputJsonPath>` scheduled from the existing gameplay automation area in `T66PlayerController_Overlays.cpp`, which already owns T66 gameplay command-line smoke/capture modes; it will run in a direct gameplay/test-room entry, write a JSON/log summary, and exit.
   - The smoke proof needs to cover:
     ...
     - stale save/inventory IDs `Item_Accuracy`, `Item_Cheating`, `Item_Stealing`, `Item_HpRegen`, and `Item_LifeSteal` are skipped/removed by the chosen warn-and-skip policy without a crash.
