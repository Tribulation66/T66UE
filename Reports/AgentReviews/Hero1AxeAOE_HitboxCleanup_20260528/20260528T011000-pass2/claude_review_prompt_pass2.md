...
Review scope:
- Packet path: C:\UE\T66\Reports\AgentReviews\Hero1AxeAOE_HitboxCleanup_20260528\plan_packet_pass2.md
- Output scope: review of the packet below only.

<review_packet>
# Hero 1 Axe AOE Hitbox Alignment And VFX Cleanup Plan Packet - Pass 2
...
Claude pass 1 returned `Verdict: REVISE` at:

`Reports/AgentReviews/Hero1AxeAOE_HitboxCleanup_20260528/20260528T010311-pass1/claude_review_pass1.md`

This pass resolves the concrete objections. The active goal and continuation instructions require movement toward exact visual/hitbox alignment, so this packet treats the intended gameplay result as a true crescent-band AOE: enemies inside the inner hollow are not hit unless they are the selected primary target that starts the attack. This is not a debug-only outline pass.
...
- `Content/Data/CombatVFXBindings.csv` currently has `BaseVisualRadius=435.0`. This will be changed to `411.4` so `EffectiveSlashRadius / BaseVisualRadius` maps the production Niagara carrier to the same outer radius used by the logical hitbox.
...
If NO, why:
User approval required before proceeding: NO, because the active continuation objective requires exact visual/hitbox alignment and rejects narrower safer substitutions.
...
Required mechanisms:
  1. Mechanism: Shared visual/logical geometry contract
     Required: YES
     Planned implementation: Add `AoeInnerRadiusRatio` to `FWeaponData`; set `Hero_1_black_aoe` to `0.54`; derive the hitbox inner radius from `EffectiveSlashRadius * AoeInnerRadiusRatio`; set `BaseVisualRadius=411.4`.
     Evidence needed: source/data diff, log line, and debug frame showing outer and inner sector lines around the crescent.
...
- Discriminator: proof must show both outer and inner debug boundaries for the crescent-band shape, plus at least one target inside the visible band hit and at least one target inside the old filled-sector hollow not hit.

## Planned Edit Scope

Code/data:

- `Source/T66/Data/T66DataTypes.h`: add `AoeInnerRadiusRatio` to `FWeaponData`; document `0.0` as filled-sector sentinel.
- `Scripts/SetupWeaponsDataTable.py`: add `AoeInnerRadiusRatio` to generated CSV fields; set only row `Hero_1_black_aoe` to `0.54`, all other generated weapon rows to `0.00`.
- `Content/Data/Weapons.csv`: generated source CSV will include `AoeInnerRadiusRatio`; exact changed row is `Hero_1_black_aoe`.
- `Content/Data/DT_Weapons.uasset`: expected generated DataTable touch after setup script reload.
- `Source/T66/Gameplay/T66CombatComponent.cpp`: compute `EffectiveSlashInnerRadius`, pass it to `BuildSlashTargets`, and log `EffectiveSlashInnerRadius`/`AoeInnerRadiusRatio` with `CombatVFXProductionSpawned` or adjacent combat geometry proof logging.
...
- `Scripts/ValidateCombatVFXProductionBindings.py`: add guards for `AoeInnerRadiusRatio`, `BaseVisualRadius=411.4`, and required source fragments.
...
No other `.uasset` changes are planned. If Unreal saves dependent assets unexpectedly, stop and inspect before accepting them.
...
- outer radius: about `EffectiveSlashRadius ~= 437.5 cm` in baseline proof,
...
|---|---:|---|---|
| `Primary` | `(0, 0)` | Hit | Primary target remains hit by attack selection even though it is at center. |
...
| `InnerHollow` | `Forward * 120` | Miss | Inside old filled sector but inside new hollow. |
...
Avoid broad Git/LFS scans. Use narrow checks:
...
- For `.uasset` paths only, run `git diff --numstat -- Content/Data/DT_Weapons.uasset Content/Data/DT_CombatVFXBindings.uasset` after reload to confirm only the expected binary assets changed.
...
3. Run `Scripts/ValidateCombatVFXProductionBindings.py` through Unreal and confirm the geometry-contract guard passes.
...
7. Inspect evidence contact sheet or selected frame for visible outer and inner debug lines around the current crescent visual.
...
- restore `Hero1Axe_AOE_Base` `BaseVisualRadius` to `435.0` only if visual scale proof requires the older hand-tuned value,
- rerun the two DataTable setup scripts and validator.

## Codex Position Before Claude Review

Claude was correct that pass 1 left the gameplay semantics decision implicit. The active goal now resolves this toward true visual/hitbox equivalence. The chosen implementation uses existing combat authority and existing debug geometry hooks rather than adding Niagara collision, actor-side helper geometry, or a visual-only line. The highest-risk change is the new inner hollow miss behavior; the proof target contract makes that behavior explicit and reversible.
