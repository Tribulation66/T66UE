...
- A working-tree VFX cleanup and handoff is prepared: it documents the live tree, hitbox contract, proof bundle, and untracked-file caveats. This is a pre-commit handoff, not a fresh-clone guarantee, until the untracked infrastructure/report files are committed or otherwise restored in the next session.
- The next-agent handoff is prepared for normal item/stat confirmation, with idol overlay VFX scoped for a follow-on design pass after that route is proven or blocked.
