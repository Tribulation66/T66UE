...
Root approval clause verified live in `C:\UE\T66\AGENTS.md` on 2026-05-28:

- `C:\UE\T66\AGENTS.md:143`: "`Verdict: APPROVE`: the reviewed plan or output is greenlit for the reviewed scope. Report the review conclusion briefly, then proceed to implementation without asking for redundant manual user approval unless..."
- `C:\UE\T66\AGENTS.md:154`: "A valid Claude greenlight authorizes implementation after that report, without a separate manual approval step, unless..."
- `C:\UE\T66\AGENTS.md:175`: "A valid Claude `Verdict: APPROVE` authorizes implementation unless the user or packet explicitly holds it; a Codex fallback approval still requires manual user confirmation."

This packet does not contain a `Product Decision Required` section and the user has not marked the work planning-only, so a valid Claude `Verdict: APPROVE` applies to this multi-workstream implementation scope.
...
- The rich basic-mob routing branch and basic-mob routing CVars are deprecated, not deleted. Deletion is tracked for the B.14 cleanup/finalization pass after B.13 mob HISM validates; the pending issue update from this pass will explicitly list the deprecated paths and CVar names so the cleanup has an owner and grep target.
...
- The CVar definitions and affected branches use deterministic non-dated marker shapes: `// DEPRECATED-BASIC-LW: lightweight-only basic mobs; inert until B.14 cleanup removes this path.` For touch overlap, the marker is `// DEPRECATED-TOUCH-OVERLAP: lightweight touch damage path is fixed; inert until B.14 cleanup removes this diagnostic toggle.`

Manager tick policy:

- `UT66MobManagerSubsystem` intentionally remains a tickable `UWorldSubsystem` through `FTickableGameObject`.
- Current tick behavior is `UT66MobManagerSubsystem::IsTickable() == !ActiveMobs.IsEmpty()`, `IsTickableWhenPaused() == false`, `IsTickableInEditor() == false`, with no custom tick interval configured and no actor/component tick group. The engine tickable pass runs the manager once per frame while active mobs exist.
- The runtime "no tick" proof excludes this manager subsystem tick by design. It proves that live lightweight mob actors and their components do not tick, while the manager tick remains enabled and owns movement/status/combat/VAT advancement.
...
B.13 custom-data-ready VAT layout:

- This pass defines the manager-owned flat layout that B.13 will consume; no older B.13 field order is assumed elsewhere.
- Per active mob, the manager stores contiguous VAT data in this order: `Frame`, `StartFrame`, `EndFrame`, `ClipIndex`, `PlayRate`, `Flags`.
- The current MID application consumes `Frame`, `StartFrame`, and `EndFrame`. `ClipIndex`, `PlayRate`, and `Flags` are retained for proof/debug and B.13 HISM custom-data expansion.
- B.13 may add rendering-only packing, but it should not need to move VAT ownership back to the actor.

Expected forced VAT proof clips:

- Move samples should be naturally reachable during chase for Melee, Rush, Flying, and Ranged.
- Idle, attack, and death samples may require the non-shipping proof hook to place a mob in a controlled state; they should not be declared absent without first attempting the proof hook.
- Freeze/status samples are separate non-advancement proof and do not replace idle/move/attack/death advancement proof.
...
- Mark routing CVars and rich-basic-mob routing branch with `// DEPRECATED-BASIC-LW: lightweight-only basic mobs; inert until B.14 cleanup removes this path.` Deletion is deferred to B.14 cleanup/finalization after B.13 validation.
...
- Neutralize `T66.Mob.Diagnostics.UseTouchDamageOverlap`; mark it with `// DEPRECATED-TOUCH-OVERLAP: lightweight touch damage path is fixed; inert until B.14 cleanup removes this diagnostic toggle.` Delete later in B.14.
- Preserve visual behavior and reset on pool reuse.
- Keep `UT66MobManagerSubsystem` ticking once per engine frame while active mobs exist; do not add actor/component tick to replace manager tick.
...
- The kill-mid-flight proof is deterministic: a non-shipping proof hook creates or reuses a boss actor as projectile source, calls `UT66ProjectileManagerSubsystem::FireBossProjectile(...)` with a fixed origin/direction/speed/lifetime so the projectile is in flight but not intersecting the hero, records hero HP/damage counters, directly destroys the boss source before the next projectile-manager tick rather than using the normal damage-to-zero path, advances the manager for fixed ticks, then asserts `DroppedInvalidSource >= 1`, `ProjectilesHitHero` did not increase after source death, and hero HP did not decrease after source death. It must not rely on natural boss fire timing.
...
- Trap projectile fire/impact logs: demote routine `LogT66TrapProjectile` telemetry in `T66TrapArrowProjectile.cpp`, specifically `[ProjectileFired]` and routine `[ProjectileImpact]` lines, to `VeryVerbose`; preserve warnings/errors.
...
- "Reports failure" means the widget leaves the Gambler tab open, keeps the relevant button state driven by `IsBossActive()`, sets a localized/status-line failure message if one exists in the widget surface, and logs a warning if anger is at 100% but the player-controller helper could not spawn.
...
Run three lightweight-only `enemywaveperf` captures on a content-complete staged Win64 Development standalone binary before implementation. Before staging, write a source manifest for all routing, VAT, projectile manager, autocapture HP, and capture-profile files that affect this pass. Manifest columns are `Path`, `SHA256`, `LengthBytes`, `LastWriteTimeUtc`, and `Role`. Reuse of the previous lightweight rows is explicitly not part of the formal Phase 1 gate.
...
- Three accepted rows.
- Absolute FPS health floor: Phase 1 median must be at least `170.0 FPS`. This is below the existing observed lightweight median (`190.6166717231366 FPS`) but high enough to reject a materially unhealthy saturated run. A single high row cannot satisfy the gate; the median is the authoritative statistic.
...
- Per-row projectile sanity floor: each accepted lightweight row must record `ProjectileManagerFired >= 10` and `ProjectilesHitHero >= 10`. A row below either floor rejects as insufficient ranged exercise. The `10` floor is intentionally low relative to current lightweight observations (`19-21` fired/hit in prior rows): it proves Ranged delivery is active without turning this performance health check into a fire-rate tuning gate.
...
4. Run three lightweight-only `enemywaveperf` captures with the same profile as Phase 1.
   - Basic mobs lightweight unconditionally.
   - `T66.AutoCapture.HeroHPOverride=20000`.
   - `T66.Ranged.DiagnosticLogging=1` aggregate counters only.
...
8. Run runtime tick proof: no `AT66MobBase` actor tick and no lightweight component tick while mobs are live, using actual runtime `IsActorTickEnabled()` and component tick state, not header defaults. Explicitly report that `UT66MobManagerSubsystem` remains tickable and active while active mobs exist; this manager tick is expected and excluded from the no-actor/no-component-tick assertion.
...
- document rich basic-mob path deprecation and intermittent rich delivery limitation with three explicit statements: the rich basic-mob path is no longer supported for basic mobs; the original intermittent rich Ranged movement/delivery symptom remains reproducible on the deprecated path and is not fixed here; B.14 cleanup/removal cancels the limitation by deleting the unsupported path
...
- add or retain a B.14 cleanup tracker entry listing deprecated marker grep targets: `DEPRECATED-BASIC-LW`, `DEPRECATED-TOUCH-OVERLAP`, deprecated projectile actor classes, and any neutralized CLI readbacks
...
- Runtime proof: no `AT66MobBase` actor tick and no lightweight component tick while mobs live; `UT66MobManagerSubsystem` remains tickable once per frame while active mobs exist and is explicitly reported as the intended central tick owner.
