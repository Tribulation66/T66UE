...
Yes by code-path and validator inspection, for this current working tree/editor asset state, but I did not run a fresh manual altar-click capture in this answer pass.

The exact condition is: if the weapon altar is offering the black-rarity Hero 1 AOE weapon, `Ashfall Cleaver` / `Hero_1_black_aoe`, and you choose it, the weapon manager equips that weapon ID. When the hero then fires an AOE auto-attack at an enemy, `UT66CombatComponent` uses the equipped weapon data, resolves the runtime `DT_CombatVFXBindings` row `Hero1Axe_AOE_Base`, and spawns `/Game/VFX/Hero1/Axe/AOE/NS_Hero1AxeAOE_MeshSlash`.
...
- Validator warnings were unrelated to this row/path: `r.Upscale.Quality` scalability priority and the existing ToonStyle material include warning. The commandlet ended with `Success - 0 error(s), 3 warning(s)`.
