...
Working-tree-only, code-verified answer: the binding is wired so it should work, but I have not done a fresh manual altar-click capture in this answer pass, so I would not call it end-to-end playthrough-confirmed yet.

The important caveat is first: `DT_CombatVFXBindings.uasset`, `CombatVFXBindings.csv`, `NS_Hero1AxeAOE_MeshSlash.uasset`, and the setup/validator scripts are untracked right now. In this same working tree they exist and validate; in a clean checkout of current `HEAD`, this VFX path is not guaranteed and likely will not reproduce until those files are staged/committed or restored.

Given that working-tree state, if the weapon altar is showing the black-rarity Hero 1 AOE weapon, `Ashfall Cleaver` / `Hero_1_black_aoe`, choosing it should equip that exact weapon row and the next AOE auto-attack should spawn `/Game/VFX/Hero1/Axe/AOE/NS_Hero1AxeAOE_MeshSlash`. If the altar resolves to another rarity, that will build a different weapon ID and will not use this binding unless that other rarity gets its own binding row.

The exact route is: choosing `Hero_1_black_aoe` makes the weapon manager equip that ID. When the hero then fires an AOE auto-attack at an enemy, `UT66CombatComponent` uses the equipped weapon data, resolves the runtime `DT_CombatVFXBindings` row `Hero1Axe_AOE_Base`, and spawns the bound Niagara system.

Important caveats:

- The answer is code-path and validator verified, not fresh manual-playthrough verified.
- This is specifically for the black-rarity AOE offer. Other rarity tiers would build different weapon IDs and do not point at this `Hero_1_black_aoe` binding unless they get their own binding rows.
