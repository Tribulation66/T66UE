...
- Stage 17 Four Horsemen validation scope: the smoke may set a validation-only entry state equivalent to starting Impossible difficulty at global stage 17 (`T66GI->SelectedDifficulty = Impossible`, `RunState->SetCurrentStage(17)`) and then call `SpawnBossForCurrentStage()`. This intentionally bypasses playing stages 1-16 and is not progression proof. It is acceptable only as a boss projectile capacity/color smoke because it still uses production stage data, `BossEncounterID`, `BossEncounterMembers`, boss spawning, `Awaken()`, and `FireAtPlayer()` paths.
...
- Cap rationale: `Content/Data/Bosses.csv` currently has 24 boss rows and Stage 17 uses four distinct profile/color rows. A cap of 32 exact buckets covers one exact bucket per current boss row plus headroom without permitting unbounded component growth. Secondary-tint or future authored colors can overflow deterministically instead of growing components without limit.
...
- Concrete death test hook: add a `#if !UE_BUILD_SHIPPING` smoke option such as `T66BossProjectileSmokeKillMidFlight=1`. After managed boss projectiles have fired and before their six-second lifetime expires, the automation calls the existing boss damage path (`Boss->TakeDamageFromHeroHit(Boss->CurrentHP + Boss->MaxHP + 1000, ...)`) on one awakened boss. The check passes only if no later `[CombatDamage] Delivery=BossProjectile` is attributed to that dead/invalid boss and the process exits cleanly.
...
- Enemy-spit parity:
  - existing rich/lightweight Ranged calls already pass lifetime `4.f` into `UT66ProjectileManagerSubsystem::FireProjectile(...)`.
  - this pass must preserve that value exactly for enemy-spit projectiles. The boss wrapper uses `6.f` to match `AT66BossProjectile::InitialLifeSpan`.
- Tick/update cost note:
  - boss HISM body transforms already need per-frame position updates for movement; adding velocity-derived rotation to the same transform update is expected to be negligible compared with the old per-projectile actor tick/component stack.
  - smoke should record `ActivePeak`; if the Four Horsemen peak is unexpectedly near capacity or transform updates visibly hitch, that becomes a follow-up performance issue.
...
- Add `FireBossProjectile(const FT66ManagedProjectileFireParams& Params)` as the boss-facing API.
- Keep the existing `FireProjectile(...)` signature for enemy-spit call sites and implement it as a compatibility wrapper that builds an `FT66ManagedProjectileFireParams` with `Delivery=EnemyProjectile`, the caller-supplied lifetime passed through unchanged (current rich/lightweight Ranged call sites pass `4.f`), and type `EnemySpitProjectileTypeIndex`.
- The underlying source of truth is the shared structured allocation path; boss code calls the boss wrapper, existing mob/rich-ranged code can keep the old signature unless a narrow call-site cleanup is clearer.
...
- Do not use `UCLASS(..., Deprecated)` in this pass. The class is deliberately retained for asset/reference compatibility, and class-level deprecation can alter Blueprint/load behavior.
- Add `UE_DEPRECATED(5.7, "Boss projectiles are managed by UT66ProjectileManagerSubsystem.")` to C++ constructor/helper declarations only if compatible with UHT/compile.
- If constructor/helper deprecation causes UHT/compiler incompatibility, keep the comment and document the attempted stronger deprecation in the completion packet rather than breaking the build.
...
- MultiBoss path:
  ...
  - sets `T66GI->SelectedDifficulty` to `Impossible` and `RunState->SetCurrentStage(17)` inside the non-shipping automation hook, then calls the existing production `AT66GameMode::SpawnBossForCurrentStage()` path so `BossEncounterID` and `BossEncounterMembers` drive the Four Horsemen spawn,
...
Do not manually spawn the four boss IDs outside `SpawnBossForCurrentStage()` unless review is rerun. The point of this smoke is production-path MultiBoss boss/projectile validation, not stage-progression validation. The completion packet must state that stages 1-16 are intentionally bypassed for this capacity/color smoke.

Death-mid-flight option:

- Add `T66BossProjectileSmokeKillMidFlight=1`.
- After `ProjectilesFired > 0` is observed or after a short fixed delay following awakening, damage one awakened boss through `TakeDamageFromHeroHit(...)` for lethal damage.
- Continue the smoke until the original in-flight projectiles would have had time to hit/expire, then assert no stale-source boss damage and no crash.
...
- `Gameplay/MASTER_PLAYER_EXPERIENCE.md` and other UX/player-experience docs are intentionally untouched unless implementation reveals player-facing rule changes beyond projectile ownership; this pass should preserve gameplay patterns and only move projectile infrastructure.
...
- Run the same smoke with `T66BossProjectileSmokeKillMidFlight=1` and confirm in-flight projectiles deactivate/drop safely without crashing or applying damage with a stale owner.
- Confirm enemy-spit projectiles still use the existing 4-second lifetime in code review or log evidence.
- Save:
...
- Confirm:
  - Stage 17 setup is limited to non-shipping validation (`SelectedDifficulty=Impossible`, `CurrentStage=17`) and four boss IDs are spawned through `SpawnBossForCurrentStage()` from Stage 17 encounter data and awakened through the boss-gate path,
...
- no helper or wrapper with a `T66BossProjectile_Spawn`-style name remains as a production boss fire path.
