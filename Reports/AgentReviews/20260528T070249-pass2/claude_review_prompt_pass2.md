...
- A concrete component-tick table for `AT66MobBase` and any subclass used by the four lightweight families. The table must list, at minimum:
  ...
  - Any `UCharacterMovementComponent`, custom movement component, timeline, curve helper, or latent per-frame helper attached to lightweight mobs. If absent, state absent with evidence.
...
- A narrow documentation sweep for existing docs that assert VAT or actor-tick ownership, including `C:\UE\T66\Gameplay\Combat\MASTER_COMBAT.md` if present and any lightweight-mob master docs found by targeted search.

### Stage 0: Establish Current Clean Baseline And Close B.10 Acceptance

Run against the existing staged standalone binary if one exists and is runnable. Record its SHA256 before the first capture and treat that SHA as the B.10 closure artifact. Rebuild/stage before Stage 0 only if no usable staged binary exists; if a rebuild is needed, record that explicitly and use the new staged hash as the baseline artifact.
...
- binary hash at pass start, before/after every capture, and pass end. The B.10.1D runner already records executable fingerprint data in progress/result artifacts; if a new runner is created, it must preserve equivalent per-capture pre/post hash fields.
- clean environment before each set and capture.
- no per-frame ranged diagnostic lines.
- expected placed Slime guardian/miniboss route attribution must be documented and explicitly excluded from route-validity rejection. Random miniboss promotion remains invalid; the placed guardian is not a leak.
...
- Preserve rows/logs as diagnostic evidence.
...
- Capture runtime proof of actor tick disabled, either through an existing automation dump/summary or a one-shot verification log/assert that reports `IsActorTickEnabled() == false` for sampled lightweight mobs. Do not add per-frame logging.
