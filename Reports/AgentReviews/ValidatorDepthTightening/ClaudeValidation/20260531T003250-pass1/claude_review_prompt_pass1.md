...
- `Scripts\Invoke-ClaudePlanReview.ps1:32-44` exposes `-MaxTurns` from 1 to 10 and Claude `-Effort` from low through max.
...
- Invoke Claude validation with `-Effort high` or `-Effort xhigh` for broad architecture/change plans. The helper currently allows up to `-MaxTurns 10`, and the default is already 10.
