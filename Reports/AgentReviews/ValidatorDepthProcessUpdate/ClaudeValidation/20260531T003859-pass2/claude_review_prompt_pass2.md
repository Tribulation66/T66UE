...
- When `ReviewDepth` is `deepened`, augment the Claude prompt with explicit risk/oversight instructions and add extra review headings without removing the strict first-line verdict or existing headings.
- Increase effective effort to at least `high` for deepened review only when the caller did not explicitly pass `-Effort`, using `$PSBoundParameters.ContainsKey('Effort')` to preserve an explicit `-Effort low`.
- Preserve the existing Anthropic API key billing guard exactly. The auto-raise runs only through the existing subscription-backed Claude CLI path after the guard passes, and the helper output reports the requested and effective effort so usage remains visible.
...
- Run the updated helper on the final change packet with `-RiskReview` to prove the new prompt path works and Claude can return a valid verdict with the strict verdict-line-first contract.
- Run a dry parse against the produced review with `-ParseReviewPathOnly`.
- Run a prompt-generation check for both branches of effort handling: one deepened review with no explicit `-Effort` should report effective effort `high`; one deepened review with explicit `-Effort low` should preserve effective effort `low`.
