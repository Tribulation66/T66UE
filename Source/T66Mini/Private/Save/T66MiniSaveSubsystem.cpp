...
UT66MiniRunSaveGame* UT66MiniSaveSubsystem::CreateSeededRunSave(const UT66MiniFrontendStateSubsystem* FrontendState) const
{
	...
	RunSave->WaveIndex = 1;
	RunSave->CurrentStageID = NAME_None;
	RunSave->StageIndex = 1;
	RunSave->HeroLevel = 1;
...
UT66MiniProfileSaveGame* UT66MiniSaveSubsystem::LoadOrCreateProfileSave(const UT66MiniDataSubsystem* DataSubsystem) const
{
	...
	if (!bHasResolvedProfileSave)
	{
		...
		if (!ProfileSave)
		{
			...
			ProfileSave->BestStageIndexReached = 1;
	...
	auto ClampField = [&bChanged](int32& Field, int32 MinValue) { const int32 New = FMath::Max(MinValue, Field); if (Field != New) { Field = New; bChanged = true; } };
	ClampField(ProfileSave->CompletedRunCount, 0);
	ClampField(ProfileSave->FailedRunCount, 0);
	ClampField(ProfileSave->BestWaveReached, 1);
	ClampField(ProfileSave->BestStageIndexReached, 1);
	ClampField(ProfileSave->LifetimeMaterialsCollected, 0);
	ClampField(ProfileSave->LifetimeGoldCollected, 0);
	ClampField(ProfileSave->TotalStagesCleared, 0);

	for (TPair<FName, int32>& Pair : ProfileSave->CompanionUnityStagesClearedByID)
	{
		ClampField(Pair.Value, 0);
	}
...
bool UT66MiniSaveSubsystem::RecordRunSummary(const FT66MiniRunSummary& Summary, const UT66MiniDataSubsystem* DataSubsystem) const
{
	...
	ProfileSave->LifetimeGoldCollected += FMath::Max(0, Summary.GoldCollected);
	ProfileSave->BestWaveReached = FMath::Max(ProfileSave->BestWaveReached, Summary.WaveReached);
	ProfileSave->BestStageIndexReached = FMath::Max(ProfileSave->BestStageIndexReached, Summary.WaveReached);
