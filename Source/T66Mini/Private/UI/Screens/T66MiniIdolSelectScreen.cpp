...
namespace
{
	const FSlateBrush* T66MiniIdolSceneBackgroundBrush()
	{
	...
	TSharedRef<SWidget> T66MiniMakeIdolSceneBackground(const FLinearColor& FallbackColor)
	{
		if (const FSlateBrush* Brush = T66MiniIdolSceneBackgroundBrush())
		{
			return SNew(SImage)
				.Image(Brush)
...
TSharedRef<SWidget> UT66MiniIdolSelectScreen::BuildSlateUI()
{
	...
	if (FrontendState)
	{
		...
		StatusMetrics->AddSlot(1, 2)[MakeMetricChip(TEXT("Mode"), FString(TEXT("SOLO")), FLinearColor::White)];
	}

	const FText ContinueLabel = NSLOCTEXT("T66Mini.IdolSelect", "Continue", "CONTINUE");

	return SNew(SOverlay)
		+ SOverlay::Slot()
		[
			T66MiniMakeIdolSceneBackground(BackgroundFill)
		]
		+ SOverlay::Slot()
		[
			SNew(SBorder)
...
FReply UT66MiniIdolSelectScreen::HandleContinueClicked()
{
	...
	if (FrontendState->IsIntermissionFlow())
	{
		...
		if (DataSubsystem)
		{
			if (const FT66MiniStageDefinition* StageDefinition = DataSubsystem->FindStage(ActiveRun->DifficultyID, ActiveRun->WaveIndex))
			{
