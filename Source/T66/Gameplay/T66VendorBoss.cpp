...
void AT66VendorBoss::BeginPlay()
{
	...
	// Placeholder mesh: smaller sphere (overridden by ApplyCharacterVisual if mapped).
	...
	if (UGameInstance* GI = GetWorld() ? GetWorld()->GetGameInstance() : nullptr)
	{
		...
		{
			const int32 UnlockedTokenRank = Achievements->GetVendorTokenUnlockedLevel();
			if (UnlockedTokenRank > 0)
			{
				MetaScalar = 1.f + 0.20f * static_cast<float>(UnlockedTokenRank);
				GroundAOEBaseDamageHP = FMath::Max(1, FMath::RoundToInt(static_cast<float>(GroundAOEBaseDamageHP) * MetaScalar));
				FireIntervalSeconds = FMath::Max(0.25f, FireIntervalSeconds * (1.f - 0.05f * static_cast<float>(UnlockedTokenRank)));
				GroundAOEIntervalSeconds = FMath::Max(1.5f, GroundAOEIntervalSeconds * (1.f - 0.04f * static_cast<float>(UnlockedTokenRank)));
				if (UCharacterMovementComponent* Move = GetCharacterMovement())
				{
					Move->MaxWalkSpeed *= 1.f + 0.05f * static_cast<float>(UnlockedTokenRank);
				}
			}
		}
		...
		{
			const bool bApplied = Visuals->ApplyCharacterVisual(FName(TEXT("VendorBoss")), GetMesh(), VisualMesh, true, false, false, VisualMesh);
			if (!bApplied && GetMesh())
			{
...
void AT66VendorBoss::Die()
{
	...
	if (UGameInstance* GI = World ? World->GetGameInstance() : nullptr)
	{
		...
		if (UT66AchievementsSubsystem* Achievements = GI->GetSubsystem<UT66AchievementsSubsystem>())
		{
			Achievements->UpgradeVendorTokenForDifficulty(Difficulty);
		}
		...
		if (World)
		{
			...
			{
				TokenBag->SetItemID(FName(TEXT("Item_VendorToken")));
				TokenBag->SetLootRarity(ET66Rarity::White);
				TokenBag->SetExplicitLine1RolledValue(1);
			}
		}
	}
	Super::Die();
