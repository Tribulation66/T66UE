...
void UT66BackendSubsystem::FetchDailyLeaderboard(const FString& Filter)
{
	if (!FT66ShelvedFeatureGate::IsDailyDescentEnabled())
	{
		...
		OnLeaderboardDataReady.Broadcast(CacheKey);
		return;
	}
	...
	const FString& FilterContext)
{
	...
	else if (ResolvedFilter.Equals(TEXT("companion"), ESearchCase::IgnoreCase) && !ResolvedFilterContext.IsEmpty())
	{
		Endpoint += FString::Printf(TEXT("&companion_id=%s"), *ResolvedFilterContext);
	}
