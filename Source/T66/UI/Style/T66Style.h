...
public:
	...
	struct Tokens
	{
		...
		// Shop/Gambler shared layout -- all fixed sizes (no FillWidth/FillHeight in main row).
		// See Tokens docs or this file for full width/height list.
		static constexpr float InventorySlotSize = 160.f;   // Inventory slots on Shop and Gambler screens
		static constexpr float ItemPanelIconSize = 200.f;   // Shop item icon; Gambler game card icon
		static constexpr float GameCardMinWidth = 260.f;
		static constexpr float GameCardHeight = 200.f;
		/** Center panel width for NPC overlays (Shop + Gambler casino). */
		...
		/** Fixed height of the main 3-column row (Stats | Shop/Casino | Bank) on Shop and Gambler. */
		static constexpr float NPCMainRowHeight = 600.f;
		/** Fixed height of the inventory panel strip at the bottom on Shop and Gambler. */
		...
		/** Stats panel width (Shop and Gambler); single source of truth. */
		...
		/** Shop stats panel width (same as NPCStatsPanelWidth). */
		...
		/** Shop item card: width and height (each of the 3 cards). */
		...
		/** Root overlay padding (Shop/Gambler). */
