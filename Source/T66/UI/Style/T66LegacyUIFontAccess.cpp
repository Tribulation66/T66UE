...
namespace T66LegacyUIFontAccess
{
	...
	{
		if (!RelativeThemePath || !*RelativeThemePath)
		{
			return FString();
		}

		const FString ThemeRelativePath = FString::Printf(TEXT("RuntimeDependencies/T66/ThemeFonts/%s"), RelativeThemePath);
		return ResolveFirstExistingProjectPath({ *ThemeRelativePath });
	}
