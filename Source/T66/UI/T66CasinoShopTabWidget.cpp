...
void UT66CasinoShopTabWidget::NativeDestruct()
{
	...
	if (UWorld* World = GetWorld())
	{
		if (UGameInstance* GI = World->GetGameInstance())
		{
			...
			{
				RunState->GoldChanged.RemoveDynamic(this, &UT66CasinoShopTabWidget::HandleGoldOrDebtChanged);
				RunState->DebtChanged.RemoveDynamic(this, &UT66CasinoShopTabWidget::HandleGoldOrDebtChanged);
				RunState->InventoryChanged.RemoveDynamic(this, &UT66CasinoShopTabWidget::HandleInventoryChanged);
				RunState->ShopChanged.RemoveDynamic(this, &UT66CasinoShopTabWidget::HandleShopChanged);
				RunState->BuybackChanged.RemoveDynamic(this, &UT66CasinoShopTabWidget::HandleBuybackChanged);
				RunState->BossChanged.RemoveDynamic(this, &UT66CasinoShopTabWidget::HandleBossChanged);
			}
		}
	}
...
void UT66CasinoShopTabWidget::ReleaseSlateResources(bool bReleaseChildren)
{
	ReleaseCachedSlateResources();
	Super::ReleaseSlateResources(bReleaseChildren);
}

void UT66CasinoShopTabWidget::ReleaseCachedSlateResources()
{
	PageSwitcher.Reset();
	NetWorthText.Reset();
	GoldText.Reset();
...
void UT66CasinoShopTabWidget::CloseOverlay()
{
	if (UWorld* World = GetWorld())
	{
		World->GetTimerManager().ClearTimer(StealTickTimerHandle);
...
TSharedRef<SWidget> UT66CasinoShopTabWidget::RebuildWidget()
{
	...
	if (RunState)
	{
		...
		RunState->GoldChanged.AddDynamic(this, &UT66CasinoShopTabWidget::HandleGoldOrDebtChanged);
		RunState->DebtChanged.AddDynamic(this, &UT66CasinoShopTabWidget::HandleGoldOrDebtChanged);
		RunState->InventoryChanged.AddDynamic(this, &UT66CasinoShopTabWidget::HandleInventoryChanged);
		RunState->ShopChanged.AddDynamic(this, &UT66CasinoShopTabWidget::HandleShopChanged);
		RunState->BuybackChanged.AddDynamic(this, &UT66CasinoShopTabWidget::HandleBuybackChanged);
	...
	if (TexPool)
	{
		...
		T66SlateTexture::BindBrushAsync(TexPool, HappySoft, this, AngerFace_Happy, FName(TEXT("ShopFaceHappy")), /*bClearWhileLoading*/ true);
		T66SlateTexture::BindBrushAsync(TexPool, NeutralSoft, this, AngerFace_Neutral, FName(TEXT("ShopFaceNeutral")), /*bClearWhileLoading*/ true);
		T66SlateTexture::BindBrushAsync(TexPool, AngrySoft, this, AngerFace_Angry, FName(TEXT("ShopFaceAngry")), /*bClearWhileLoading*/ true);
	}
...
void UT66CasinoShopTabWidget::HandleInventoryChanged()
{
...
void UT66CasinoShopTabWidget::HandleShopChanged()
{
	PrimeVisibleItemIconTextures();
	RefreshShopChrome();
	RefreshTopBar();
...
void UT66CasinoShopTabWidget::HandleBuybackChanged()
{
	PrimeVisibleItemIconTextures();
	RefreshShopChrome();
	RefreshTopBar();
...
void UT66CasinoShopTabWidget::RefreshAll()
{
	PrimeVisibleItemIconTextures();
	RefreshShopChrome();
	RefreshTopBar();
...
void UT66CasinoShopTabWidget::RefreshShopChrome()
{
	UWorld* World = GetWorld();
	UT66LocalizationSubsystem* Loc = World && World->GetGameInstance()
		? World->GetGameInstance()->GetSubsystem<UT66LocalizationSubsystem>()
		: nullptr;
	const FText ShopTitle = Loc ? Loc->GetText_Shop() : NSLOCTEXT("T66.Shop", "Shop", "SHOP");
	const FText BuybackTitle = NSLOCTEXT("T66.Shop", "Buyback", "BUYBACK");
	const bool bShowingBuyback = ShopBuybackSwitcher.IsValid() && ShopBuybackSwitcher->GetActiveWidgetIndex() == 1;
...
void UT66CasinoShopTabWidget::PrimeVisibleItemIconTextures()
{
	UWorld* World = GetWorld();
	UT66RunStateSubsystem* RunState = GetRunStateFromWorld(World);
...
void UT66CasinoShopTabWidget::RefreshStatsPanel()
{
...
void UT66CasinoShopTabWidget::RefreshTopBar()
{
	UWorld* World = GetWorld();
	UT66RunStateSubsystem* RunState = GetRunStateFromWorld(World);
	if (!RunState) return;
	...
	{
		const int32 Anger = FMath::Clamp(RunState->GetShopAngerGold(), 0, UT66RunStateSubsystem::ShopAngerThresholdGold);
		const int32 Pct = FMath::RoundToInt((static_cast<float>(Anger) / static_cast<float>(UT66RunStateSubsystem::ShopAngerThresholdGold)) * 100.f);
		if (Pct >= 67)
		{
...
FReply UT66CasinoShopTabWidget::OnReroll()
{
	if (UWorld* World = GetWorld())
	{
...
void UT66CasinoShopTabWidget::RefreshStock()
{
	UWorld* World = GetWorld();
	UT66RunStateSubsystem* RunState = GetRunStateFromWorld(World);
	if (!RunState) return;
...
void UT66CasinoShopTabWidget::RefreshBuyback()
{
	UWorld* World = GetWorld();
	UT66RunStateSubsystem* RunState = GetRunStateFromWorld(World);
	if (!RunState) return;
...
void UT66CasinoShopTabWidget::SetPage(EShopPage Page)
{
...
void UT66CasinoShopTabWidget::OpenShopPage()
{
	SetPage(EShopPage::Shop);
	if (ShopBuybackSwitcher.IsValid())
	{
...
void UT66CasinoShopTabWidget::RefreshInventory()
{
	UWorld* World = GetWorld();
	UT66RunStateSubsystem* RunState = GetRunStateFromWorld(World);
	if (!RunState) return;
	...
	for (int32 i = 0; i < InventorySlotTexts.Num(); ++i)
	{
		...
		const int32 StackCount = (bHasItem && InvSlots.IsValidIndex(i) && InvSlots[i].IsValid())
			? StackCounts.FindRef(MakeShopInventoryStackKey(InvSlots[i]))
			: 0;
...
void UT66CasinoShopTabWidget::RefreshSellPanel()
{
	UWorld* World = GetWorld();
	UT66RunStateSubsystem* RunState = GetRunStateFromWorld(World);
	if (!RunState) return;
...
FReply UT66CasinoShopTabWidget::OnBack()
{
	CloseOverlay();
	return FReply::Handled();
}

FReply UT66CasinoShopTabWidget::OnBorrowClicked()
{
	UWorld* World = GetWorld();
	UT66RunStateSubsystem* RunState = GetRunStateFromWorld(World);
...
FReply UT66CasinoShopTabWidget::OnPaybackClicked()
{
	UWorld* World = GetWorld();
	UT66RunStateSubsystem* RunState = GetRunStateFromWorld(World);
	if (!RunState) return FReply::Handled();
...
FReply UT66CasinoShopTabWidget::OnSelectInventorySlot(int32 InventoryIndex)
{
...
FReply UT66CasinoShopTabWidget::OnSellSelectedClicked()
{
	UWorld* World = GetWorld();
	UT66RunStateSubsystem* RunState = GetRunStateFromWorld(World);
	if (!RunState) return FReply::Handled();
...
FReply UT66CasinoShopTabWidget::OnBuySlot(int32 SlotIndex)
{
	UWorld* World = GetWorld();
	UT66RunStateSubsystem* RunState = GetRunStateFromWorld(World);
	if (!RunState) return FReply::Handled();
...
FReply UT66CasinoShopTabWidget::OnBuybackSlot(int32 SlotIndex)
{
...
FReply UT66CasinoShopTabWidget::OnStealSlot(int32 SlotIndex)
{
	if (!bShopAllowsSteal)
	{
...
void UT66CasinoShopTabWidget::ShowStealPrompt(int32 SlotIndex)
{
	...
	if (UWorld* World = GetWorld())
	{
		...
		World->GetTimerManager().SetTimer(StealTickTimerHandle, this, &UT66CasinoShopTabWidget::TickStealBar, 1.f / 15.f, true);
	}
}

void UT66CasinoShopTabWidget::HideStealPrompt()
{
...
void UT66CasinoShopTabWidget::TickStealBar()
{
...
FReply UT66CasinoShopTabWidget::OnStealStop()
{
	UWorld* World = GetWorld();
	UT66RunStateSubsystem* RunState = GetRunStateFromWorld(World);
	if (!RunState)
	...
	if (StatusText.IsValid())
	{
		...
		if (Outcome == ET66ShopStealOutcome::Miss) StatusText->SetText(NSLOCTEXT("T66.Shop", "StealFailedMiss", "Steal failed (miss). Anger increased."));
		else if (Outcome == ET66ShopStealOutcome::Failed) StatusText->SetText(NSLOCTEXT("T66.Shop", "StealFailed", "Steal failed. Anger increased."));
		...
		else StatusText->SetText(NSLOCTEXT("T66.Shop", "StealFailed", "Steal failed. Anger increased."));
	}

	RefreshAll();
	TriggerCasinoBossIfAngry();
	return FReply::Handled();
}

bool UT66CasinoShopTabWidget::IsBossActive() const
{
	return bCachedBossActive;
}

void UT66CasinoShopTabWidget::HandleBossChanged()
{
...
void UT66CasinoShopTabWidget::TriggerCasinoBossIfAngry()
{
	if (AT66PlayerController* PC = Cast<AT66PlayerController>(GetOwningPlayer()))
	{
