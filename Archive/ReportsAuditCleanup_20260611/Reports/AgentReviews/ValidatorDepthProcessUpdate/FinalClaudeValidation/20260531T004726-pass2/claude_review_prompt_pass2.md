...
- When deepened review is requested and the caller does not pass `-Effort`, the helper auto-raises effective effort to `high`.
...
- `AGENTS.md` now routes broad/high-rework-risk/process/migration/deletion/runtime-source-sync packets to deepened risk-focused validation and mentions `-ReviewDepth deepened` / `-RiskReview`.
...
1. `Scripts\Invoke-ClaudePlanReview.ps1`
   ...
   - Added effective effort handling using `$PSBoundParameters.ContainsKey("Effort")`.
   - Added helper output fields: `ReviewDepth`, `RiskReview`, `RequestedEffort`, and `EffortSource`; existing `Effort` now reports the effective CLI effort.
...
3. `AGENTS.md`
   ...
   - Added the helper flags to the validator review registry row.
...
- `Scripts\Test-ClaudeReviewVerdictParser.ps1 -OutputRoot Reports\AgentReviews\ValidatorDepthProcessUpdate\ParserFixtures`: PASS, 18 verdict fixtures and 6 auth fixtures.
- Smoke run with `-RiskReview` and no explicit `-Effort`: PASS. Output reported `ReviewDepth=deepened`, `RiskReview=True`, `Effort=high`, `RequestedEffort=low`, `EffortSource=AutoRaisedForDeepenedReview`, and `OutcomeKind=ClaudeValidVerdict`.
- Smoke run with `-RiskReview -Effort low`: PASS. Output reported `ReviewDepth=deepened`, `RiskReview=True`, `Effort=low`, `RequestedEffort=low`, `EffortSource=Explicit`, and `OutcomeKind=ClaudeValidVerdict`.
...
- Accepted combined alias test with `-RiskReview -ReviewDepth deepened -Effort low`: PASS. Output reported `ReviewDepth=deepened`, `RiskReview=True`, `Effort=low`, `EffortSource=Explicit`, and `OutcomeKind=ClaudeValidVerdict`.
- Direct primary flag test with `-ReviewDepth deepened`: PASS. Output reported `ReviewDepth=deepened`, `RiskReview=True`, `Effort=high`, `RequestedEffort=low`, `EffortSource=AutoRaisedForDeepenedReview`, and `OutcomeKind=ClaudeValidVerdict`.
...
- Known caveats: automatic enforcement for all broad packets was not implemented; the new behavior is protocol-directed and helper-supported via `-ReviewDepth deepened` or `-RiskReview`. Deepened validation now auto-raises default helper effort to `high` when no `-Effort` is supplied; this is intentionally surfaced in both docs and helper output, and callers can override it with an explicit `-Effort`.
...
- Decisions the Validator should challenge: whether the helper should have changed parser behavior; whether auto-raising default effort is acceptable; whether the `AGENTS.md` routing text is sufficient.
