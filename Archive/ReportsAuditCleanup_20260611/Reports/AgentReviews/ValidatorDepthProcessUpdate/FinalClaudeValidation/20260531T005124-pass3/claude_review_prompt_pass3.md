...
- Known caveats: automatic enforcement for all broad packets was not implemented; the new behavior is protocol-directed and helper-supported via `-ReviewDepth deepened` or `-RiskReview`. Deepened validation does not silently raise helper effort; callers should pass `-Effort high`, `-Effort xhigh`, or `-Effort max` explicitly when a packet warrants more model thinking.
