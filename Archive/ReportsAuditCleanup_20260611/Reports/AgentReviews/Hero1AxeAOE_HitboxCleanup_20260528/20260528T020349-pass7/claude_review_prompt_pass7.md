...
- `Gameplay/Combat/MASTER_COMBAT.md`: records that `Hero1Axe_AOE_Base` is generated/enforced by the setup script, checked by the validator, and that the binding/helper files may be untracked in this repository state.
- `Gameplay/Combat/pending_issues_Combat.md`: records manual selected-frame limitations and that AOE visual polish is deferred after the hitbox close.
...
- loudly preserve the working-tree caveat: setup/validator/report files may be untracked, while generated binding CSV/uasset are live on disk but not in `git ls-files`.
