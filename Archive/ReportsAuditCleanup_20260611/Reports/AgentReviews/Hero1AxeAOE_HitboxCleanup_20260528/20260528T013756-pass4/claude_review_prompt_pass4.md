...
- `Content/Data/CombatVFXBindings.csv` and `Content/Data/DT_CombatVFXBindings.uasset` are live generated binding files but may not appear in Git status in this repo state.
- A failed rerun occurred because `BuildT66VideoEvidenceBundle.py` accepts only `start`, `mid`, `impact`, and `dissipate` selected-frame labels. This was corrected by rerunning the capture with allowed labels and useful frame numbers.
