...
Verification evidence: focused compile plus Unreal-owned `hero1axeaoewateridolimpact` capture/log proof showing `WeaponPrimary` and `IdolPrimary` at `X=696.89`, `DamageBySource SourceID=Idol_Water`, and placeholder `VisualScale` reduced from `1.500` to about `0.500`.
...
Evidence: Unreal-owned capture plus `CombatVFXIdolImpactPlaceholderSpawned ... ImpactPoint=... Radius=300.00 Placeholder=BlueSphere VisualScale=0.500`.
...
3. Mechanism: Placeholder visually marks the impact point.
   ...
   Planned implementation: Change only the fallback placeholder mesh scale to `FMath::Clamp(Radius / 600.f, 0.35f, 0.65f)`, producing `0.5` for the current Water radius.
   Evidence needed: capture frame/contact sheet where the blue marker no longer covers the hero silhouette and log `VisualScale=0.500`.

## Planned Code And Doc Change

Edit `Source/T66/Gameplay/T66CombatVFX.cpp`:

1. In `SpawnWaterIdolImpactPlaceholderVFX`, change the temporary placeholder visual scale from `FMath::Clamp(Radius / 200.f, 0.80f, 1.75f)` to `FMath::Clamp(Radius / 600.f, 0.35f, 0.65f)`.
...
4. Check log lines:
   ...
   - `CombatVFXIdolImpactPlaceholderSpawned ... Radius=300.00 ... VisualScale=0.500`
