...
1. Mechanism: weapon impact context creation
   Required: YES
   Planned implementation: Add a small `FT66CombatImpactContext` in `T66CombatComponent.h` carrying source type, source ID, hero ID, attack category, attack origin, impact point, forward vector, logical radius/inner radius/sector metadata, primary target handle, hit target handles, effective damage, and optional parent/chain data. Fill one or more contexts from Pierce, AOE, Bounce, and DOT paths without changing weapon damage behavior.
...
3. Mechanism: idol-owned damage source
   Required: YES
   Planned implementation: Keep `SourceID=IdolID` when applying idol damage, but apply through target handles gathered by the idol impact context rather than direct `WeaponHitActors` actor loops for AOE idols. Water AOE will query its own sphere centered at the idol impact point and damage those target handles.
...
4. Mechanism: temporary Water placeholder at impact
   ...
   Planned implementation: Add a helper that spawns a short-lived, collisionless, blue static-mesh sphere at the Water idol impact point when no production `IdolModifier` binding exists. It must log `CombatVFXIdolImpactPlaceholderSpawned` and not write production assets.
...
6. Mechanism: proof-mode equipment and capture
   Required: YES
   Planned implementation: Add a non-shipping proof mode such as `hero1axeaoewateridolimpact` that equips Hero 1 black AOE, equips `Idol_Water`, stages fixed targets around the slash impact point, fires one real combat attack, then logs weapon damage, Water damage, placeholder spawn, and target pass/fail results. Extend `CaptureT66GameplayVideo.ps1` and likely add a wrapper script for this mode.
...
- `Source/T66/Gameplay/T66CombatComponent.cpp`
  ...
  - Replace the AOE idol `WeaponHitActors` path with context-driven AOE target query/damage for Water and future AOE idols.
  - Preserve existing non-AOE idol damage behavior as much as possible while routing through context/logging so each idol still receives an impact point and source.
  - Suppress the old temporary idol projectile lane for Water when the impact placeholder/binding path will handle Water presentation.
...
- Risk: changing idol damage selection could alter normal gameplay unexpectedly.
  - Mitigation: use context-driven target selection first for AOE idols, preserve non-AOE behavior where possible, and prove Water separately in automation.
- Risk: placeholder sphere could be mistaken for final production VFX.
  - Mitigation: log and document it as `Placeholder`; do not add a production binding row or final PPF close for Water Niagara.
- Risk: normal combat log spam.
  - Mitigation: gate impact logs behind a CVar enabled only by proof mode/capture.
- Risk: proof mode mutates normal inventory/selection state.
  - Mitigation: keep Water equip/grant inside non-shipping automation proof mode and do not change normal idol acquisition flow.
...
5. Refresh staged standalone with `Scripts/StageStandaloneBuild.ps1` and verify the taskbar shortcut target if the runtime gameplay change is accepted into the playable build.
