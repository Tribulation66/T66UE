...
5. Reward source wiring
   ...
   - Loot Wheel: apply `GetLootWheelRewardMultiplier()` only inside `AT66LootWheelInteractable::LockLootWheelReward()` after reward type selection:
     ...
     - Boost reward: multiply `BoostBonusStatPoints` and `BoostDurationSeconds` by the multiplier, rounded/clamped to sensible positive values. Examples: `1.0 => 8 points / 10s`, `2.0 => 16 points / 20s`.
