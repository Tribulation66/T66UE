...
8. Data asset reload and verification
   ...
   - Add a narrow, non-shipping command-line smoke path, because no existing hook directly proves all changed seams. The planned hook is `-T66ItemTaxonomySmoke=<OutputJsonPath>` scheduled from the existing gameplay automation area in `T66PlayerController_Overlays.cpp`; it will run in a direct gameplay/test-room entry, write a JSON/log summary, and exit.
