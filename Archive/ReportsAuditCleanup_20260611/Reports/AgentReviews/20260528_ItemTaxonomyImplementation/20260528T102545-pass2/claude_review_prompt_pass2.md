...
- Change the old head-targeting accuracy getter so it no longer depends on an item-facing `Accuracy` secondary. It should use hero base accuracy, the primary Accuracy multiplier, and the existing Headshot passive, preserving target-preference behavior without an Accuracy item.
- Implement Execute Chance per the user's previous prompt: after a crit succeeds, roll `GetExecuteChance01()` and OHKO normal enemies/mobs on success. Bosses are excluded because the user said "enemy" and existing boss kill paths should not be bypassed by an ordinary item stat without a separate boss-balance request.
...
8. If an existing automation hook can grant `Item_ExecuteChance` and force crits without adding a broad new test system, run that staged smoke; otherwise report this specific gameplay proof as skipped and name it as manual gameplay validation still needed.
