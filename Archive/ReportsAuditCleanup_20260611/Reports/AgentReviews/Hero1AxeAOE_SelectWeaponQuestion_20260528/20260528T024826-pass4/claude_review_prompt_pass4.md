...
Conditional yes: in this current working tree/editor asset state, if the weapon altar is showing the black-rarity Hero 1 AOE weapon, `Ashfall Cleaver` / `Hero_1_black_aoe`, then choosing it should equip that exact weapon row and the next AOE auto-attack should spawn `/Game/VFX/Hero1/Axe/AOE/NS_Hero1AxeAOE_MeshSlash`.

That answer is from code-path plus validator inspection, not from a fresh manual altar-click capture in this answer pass. If the altar resolves to another rarity, or if you run from a clean checkout where the untracked VFX/binding assets are missing, then this answer is not guaranteed.
...
- The binding CSV/uasset, Niagara asset, and setup/validator scripts are still untracked. A clean checkout of current `HEAD` will not reproduce this VFX path until those files are staged/committed or otherwise restored.
