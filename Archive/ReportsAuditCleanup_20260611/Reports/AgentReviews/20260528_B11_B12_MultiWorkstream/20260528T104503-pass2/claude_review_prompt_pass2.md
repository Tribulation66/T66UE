...
4. Mechanism: Pool reuse reset.
   ...
   Evidence needed: non-shipping proof output with slot/mob id, pre-release clip+frame, post-reacquire clip+frame, and reset result. Post-reacquire frame must be at clip start within `0.5` frames unless the acquired state explicitly starts in a timed override.
...
- `C:\UE\T66\Source\T66\Gameplay\T66PlayerController_Overlays.cpp` if CVar readback, autocapture dispatch, or Gambler delegation needs serialized update
...
- Gambler boss dual spawn path: centralize spawn ownership in the gameplay-owned spawn path; UI delegates to that path and no longer owns a separate boss spawn implementation.
...
7. Run runtime tick proof: no `AT66MobBase` actor tick and no lightweight component tick while mobs are live, using actual runtime `IsActorTickEnabled()` and component tick state, not header defaults.
8. Run multi-frame VAT proof: at least three samples at least `0.10s` apart per family showing frame/pose change for idle/move/attack/death where applicable, plus pool-reuse reset evidence.
9. Run Workstream VERIFICATION smokes for floors `3/4` minibosses and kill-mid-flight source invalidation.
