...
- If checking `ANTHROPIC_API_KEY` or other environment guards through PowerShell, print only scope presence/absence and never echo secret values.
...
- The deleted `Content/...` classification must explicitly enumerate known dirty deletion families if present, including `QuickReviveIcon`, `Vending`, and `Cliffs/*HillTile*` assets. They may be classified non-runtime only with evidence; they must not be silently dropped.
...
- Pablo decisions must be path-by-path, not one lump decision for a folder or category.
...
- This gate is FPS-based: higher is better, so CVar-on passes when `CVarOnMedianFPS >= 0.95 * CVarOffMedianFPS`.
...
10. The post-refactor source audit must positively assert that the body of `AT66MobBase::ForceMobVertexAnimationClipForAutomation` writes to zero removed actor VAT fields and contains no replacement actor-resident VAT state writes.
11. Preserve the current non-shipping guard for `AT66MobBase::ForceMobVertexAnimationClipForAutomation`; the focused Development compile must exercise it.
12. Confirm the eight current manager tick branches still advance VAT at most once per active mob per tick.
13. Preserve `AT66MobBase` actor tick disabled and do not introduce any component tick.
...
- If a helper is needed to drive guardian kill/open assertions, it must live inside W2-owned tower files, preferably `T66GameMode_Tower.cpp` as a non-shipping helper. It may not be added to `T66PlayerController_Overlays.cpp` or to new unlisted harness files without revising this packet or main-agent serialization.
...
- Before launching any W2 code edit, audit the existing `T66BossProjectileSmokeKillMidFlight` path in `T66PlayerController_Overlays.cpp` against `UT66ProjectileManagerSubsystem::DroppedInvalidSource` increments and confirm it can positively exercise a post-death/invalid `SourceMob` branch while a projectile is active.
- If the existing hook cannot reach that branch, W2 must stop and request a serialized main-agent overlay hook change before implementation proceeds.
...
- If boss separation remains the decision, add the source-facing note to `T66ActorRegistrySubsystem.h` near `ForEachDamageableTarget` / `GetAllDamageableTargets` at minimum, so future callers see it without reading the final packet.
...
- Do not wire a new boss broadcast to consumers outside `T66ActorRegistrySubsystem.h/.cpp` inside Workstream 3. If a consumer outside this file set needs the broadcast, stop and request serialized scope expansion.
...
- Expected existing player-controller entry point is `AT66PlayerController::TriggerCasinoBossIfAngry()`. Workstream 3 must confirm this is still the authoritative entry before removing the UI fallback; if it is missing or insufficient, stop and request a serialized main-agent controller-path edit.
...
6. Compare Stage 0b CVar-on against Stage 0a CVar-on:
   ...
   - If either side of the neutrality comparison is still only `n=3` and the result is accepted only by the noise band rather than clear improvement/no-change, escalate Stage 0b to 10 before accepting neutrality.
   - If `Stage0bMedianFPS < 0.98 * Stage0aCVarOnMedianFPS`, investigate regardless of the stdev band; noisy n=3 captures cannot launder a larger regression.
...
- Floors 2/3/4 traversal smoke from Workstream 2.
...
- Multi-frame VAT proof:
  ...
  - Sample all four lightweight families: Melee, Rush, Flying, and Ranged.
  ...
  - For scalar-dump evidence, the default is exactly the three required samples per family/clip. More samples require Pablo approval and must remain bounded; do not introduce per-frame logging.
