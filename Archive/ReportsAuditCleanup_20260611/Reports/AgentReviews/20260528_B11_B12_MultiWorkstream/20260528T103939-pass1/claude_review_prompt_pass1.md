...
The prior halted Stage 0a run produced valid lightweight rows on the pre-workstream staged binary after the missing `Hero_2_Chad/AnimatedToonStyle` content folder was added to the isolated source/content state and the standalone was restaged.
...
These rows are lightweight-only and independent of the rich CVar-off failure. This packet proposes to adopt them as Phase 1 if Claude agrees that the source/binary provenance satisfies the new lightweight-only baseline contract. If Claude rejects that reuse, Codex will rerun three lightweight-only captures before implementation.
...
- any required serialized edit to `C:\UE\T66\Source\T66\Gameplay\T66PlayerController_Overlays.cpp`
...
- source files where `T66.Mob.UseLightweight`, `T66.Mob.Diagnostics.RouteFlyingLightweight`, and `T66.Mob.Diagnostics.RouteRangedLightweight` are defined, discovered by narrow source search before edit
...
- Mark routing CVars and rich-basic-mob routing branch `// DEPRECATED`; deletion is deferred to cleanup.
...
- Neutralize `T66.Mob.Diagnostics.UseTouchDamageOverlap`; mark it `// DEPRECATED`; delete later.
...
- Add or reuse a non-shipping traversal smoke that walks floors `2 -> 3 -> 4`, kills each placed Slime guardian, and asserts descent-hole blocked while guardian lives and unblocked on death.
- Positively exercise kill-mid-flight source invalidation: force a boss projectile in flight when source boss dies and assert it is dropped or applies no post-death damage.
...
- Gambler boss dual spawn path: centralize spawn ownership in one gameplay path; UI calls only that path.
- Trap projectile fire/impact logs: demote routine telemetry to `VeryVerbose`, preserve warnings/errors.

Constraint: `T66PlayerController_Overlays.cpp` is main-agent serialized scope if Gambler cleanup requires it.
...
5. Main agent adopts the existing valid lightweight Phase 1 rows if Claude accepts them; otherwise reruns three lightweight-only captures before implementation.
...
7. Main agent integrates sub-agent outputs, builds one combined staged binary, records SHA256, and owns Phase 3 measurement/proofs.
8. Main agent updates docs/pending issues and returns one combined packet.

## Phase 1: Lightweight Baseline

Preferred: adopt the existing valid CVar-on rows from `C:\UE\T66_B11B12_Worktree\Saved\Codex\Performance\B11B12\Stage0a_PrimaryRun\capture_results.json` as Phase 1.

Fallback: rerun three lightweight-only `enemywaveperf` captures on a content-complete staged binary before implementation.
...
- Clean environment before captures: no `RunUAT`, `UnrealEditor-Cmd`, staged `T66.exe`, or `git-lfs` scan. Git status/add work from another agent is not killed; if it overlaps, record it and avoid using contaminated FPS rows unless the overlap is demonstrably outside Unreal runtime.

Acceptance:

- Three accepted rows.
- Median in healthy range. Existing baseline median is `190.6166717231366 FPS`; if rerun baseline is materially different, document why.
- Zero non-zero exits, zero overhead rejects, zero HeroDeath.
- Lightweight projectiles fire/hit in aggregate counters.
- Staged build content-complete enough to avoid missing-asset log spam; specifically confirm the `Hero_2_Chad/AnimatedToonStyle` content is present/cooked or no longer referenced.
...
- No rich miniboss/special/boss behavior changes except the Gambler spawn-path cleanup if it is in the audited file set.
...
2. Stage it content-complete; include/verify `Hero_2_Chad/AnimatedToonStyle` content to avoid missing-asset capture stalls.
...
5. Confirm Phase 3 median is within noise of Phase 1. Since the rich divorce is a routing simplification and VAT state relocation should be FPS-neutral, a small improvement is fine; a material regression triggers investigation in VAT-STATE.
6. Run runtime tick proof: no `AT66MobBase` actor tick and no lightweight component tick while mobs are live, using actual runtime `IsActorTickEnabled()` and component tick state, not header defaults.
7. Run multi-frame VAT proof: at least three samples at least `0.10s` apart per family showing frame/pose change for idle/move/attack/death where applicable, plus pool-reuse reset evidence.
8. Run Workstream VERIFICATION smokes for floors `3/4` minibosses and kill-mid-flight source invalidation.
...
- Phase 1 lightweight baseline established or adopted with valid source/staged SHA; B.10 closed with staged SHA in pending issues.
- Phase 3 lightweight median within noise of Phase 1; zero non-zero exits, zero overhead rejects, zero HeroDeath.
