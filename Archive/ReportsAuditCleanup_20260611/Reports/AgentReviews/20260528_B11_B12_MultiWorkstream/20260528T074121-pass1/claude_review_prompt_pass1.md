...
- `T66PlayerController_Overlays.cpp` must be assigned once. This packet assigns it to Workstream 3 only.
...
`T66PlayerController_Overlays.cpp` is not assigned here. Workstream 2 may inspect it but may not edit it. If Workstream 2 proves that an overlay/autocapture hook edit is unavoidable, it must stop and return a request for a serialized main-agent hook change after Workstream 3 has completed.
...
- If Workstream 2 needs `T66PlayerController_Overlays.cpp`, main serializes that edit; it is not parallelized.
...
- If Stage 0a passes, update the plan baseline table and close B.10 in `pending_issues_Gameplay.md` with the staged SHA referenced inline.
...
8. Keep `ForceMobVertexAnimationClipForAutomation` as an actor-facing non-shipping convenience only if it delegates to the manager and does not store state.
9. Confirm the eight current manager tick branches still advance VAT at most once per active mob per tick.
10. Preserve `AT66MobBase` actor tick disabled and do not introduce any component tick.
...
- Verify each placed slime guardian exists on the floor's exit hole.
...
- Save log/screenshot evidence under the final pass proof directory.
...
- Either:
  - document deliberate separation in `T66ActorRegistrySubsystem.h/.cpp` comments/API naming and leave bosses out of general damageable target iteration, or
  - include bosses only if the caller audit proves no duplicate target behavior and no extra file edits are needed.
...
- If the player-controller path needs the UI's casino NPC location behavior to avoid behavior regression, add that resolution to the player-controller-owned path in `T66PlayerController_Overlays.cpp`.
...
- Runtime tick proof:
  ...
  - If `DumpTicks` cannot be invoked with existing automation, use a Pablo-approved one-shot non-shipping hook; adding such a hook after Stage 0a invalidates the closure binary and requires the relevant measurement to be rerun on the new combined binary.
...
- Close B.10 acceptance in `C:\UE\T66\Source\T66\Gameplay\pending_issues_Gameplay.md` only if Stage 0a passed, with the staged SHA inline.
...
- Focused compile after each workstream or after integration if delegated tooling makes per-workstream compile impractical.
...
- Risk: disjoint workstreams need the same file.
  Mitigation: `T66PlayerController_Overlays.cpp` is assigned only to Workstream 3; any Workstream 2 need becomes serialized main-agent work.
...
1. Is the file ownership map sufficiently disjoint, especially the explicit `T66PlayerController_Overlays.cpp` assignment to Workstream 3?
2. Does the Stage 0a-before-implementation measurement correctly close B.10 only under a recorded source/staged-binary provenance set?
