...
Legacy status: this is not the current FriendSlop raw model path. FriendSlop raw Pixel3D imports intentionally skip this Animated ToonStyle / AccuRig-lineage bridge. Keep this document only for historical Animated ToonStyle bridge maintenance or an explicit user-approved revival.
...
- `LeapAnimation`: one-shot leap animation. Legacy Animated ToonStyle sources may still use old `Roll` source-clip names internally, but the shared runtime data column is `LeapAnimation`.
...
1. Start from the Pixal3D GLBs under:
   `Model Generation/Runs/Pixal3D/Archive/DeprecatedHeroLineups/HeroDemoLineup_20260522_AccuRig/Outputs/<HeroID>.glb`.
...
2. For demo skins and companions, this legacy bridge previously used `Model Generation/Runs/Pixal3D/HumanoidGuidelineTest_20260522_100k/Outputs/<SourceID>.glb`; that non-durable run has been cleaned. Restore/provide those sources explicitly through environment variables before running the bridge again.
...
5. The exporter uses the accepted Hero 1 Rigify/Quaternius template blend as the deformation and animation source. The reviewed template covers the gameplay movement clips; idle is regenerated as a neutral standing loop because the template idle is not an accepted gameplay idle. Do not rebuild or replace the reviewed movement actions during batch export:
   ...
   - `Leap`: legacy `Roll_LegsTorsoOnly` source when maintaining the old Animated ToonStyle bridge only
...
6. The exporter writes:
   ...
   - `<HeroID>_Walk.fbx`
   - `<HeroID>_Jump.fbx`
   - `<HeroID>_Roll.fbx` as a legacy source export that maps into `LeapAnimation`
...
3. Confirm `Content/Data/CharacterVisuals.csv` has `SkeletalMesh`, `WalkAnimation`, `IdleAnimation`, `JumpAnimation`, and `LeapAnimation` set for all 28 demo-scope rows.
