...
- Task contracts must describe the full requested end state, not a temporary clarification gate. If a decision gate is needed, ask it once and, for durable work, save it as `Reports/AgentReviews/<TaskSlug>/decision_block.md`; on continuations, reference the saved gate instead of repeating questions.
- Decision gate stop: the first valid `NEEDS_HUMAN_DECISION` verdict, or any equivalent gate where only the user can decide how the full task proceeds, is an immediate stop point. At that point, save or reference `Reports/AgentReviews/<TaskSlug>/decision_block.md`, present the decision and its choices once, and stop. Do not continue any other work while waiting for the user's answer: no packet polish, no rerunning reviews, no continuing audits, no revising docs, no further inspection, no adjacent implementation, and no asking a reworded version of the same decision.
...
- Every clarification step must make the user's opt-outs explicit: the user may say `no clarification needed` to proceed on stated assumptions, or `skip Validator review` to bypass the Operator/Validator cross-review for that request.
- If no clarification is needed, state the safe assumptions and continue to planning and Validator review. Validator review is always on for substantive answers and results unless the user has explicitly opted out.
...
- The plan must include: clarified task, user constraints, files/folders/systems to inspect or edit, applicable `*_AGENTS.md` and instruction files, intended edit scope, out-of-scope items, risks, rollback or revert considerations, and verification evidence required before completion.
- Low-risk requests may use a compact plan and a quick Validator pass. They still require Validator review before the answer or result is sent to the user; validation deepens when risk warrants it.

### Request Routing And Always-On Validation

- Every substantive answer or result is routed through the Operator/Validator stack. The Operator's answer or result is not sent to the user until the Validator validates it.
- Validation depth scales with risk. Simple questions, direct factual commands, and low-risk read-only answers get a quick validation pass. Any code/content/docs/config/tooling/workflow edit, any Unreal asset or editor work, visual/media/import/VFX/UI fidelity work, staged builds, releases, durable recommendations, or any answer where a wrong result would cause meaningful rework gets a full validation pass with PPF ceremony and a report packet as the owning process requires.
- A quick validation pass still requires a task contract and live-state checks when the answer depends on the machine or repo.
- If the validation depth is uncertain, use the fuller pass.
- A quick validation pass cannot be used to bypass a user-named workflow, accepted process, folder `*_AGENTS.md`, safety rule, credential/billing rule, or verification requirement for changed files.
- Report model token spend at the end of every substantive answer using this exact user-facing shape:
...
Token reporting is independent of validation depth. Use available host/helper token counts; when a count is not exposed, report `Unavailable`. Report `0` for Claude only when Claude was genuinely not invoked; a failed or incomplete Claude run still reports its parseable usage, otherwise `Unavailable`.

### Operator/Validator Protocol

- For requests that use the Claude/Codex Operator and Validator stack, follow `OPERATOR_VALIDATOR_PROTOCOL.md`. That file is the detailed authority for Operator packets, Codex approval, Validator checks, role routing, token-ledger rules, and Claude tool profiles.
- Core invariants remain here as routing rules: the Operator must produce a complete packet before Validator discovery; the Validator's first action is the Packet Completeness Gate; incomplete packets are returned as `Verdict: REVISE`; the Validator does targeted anchor checks by default and may deepen only under the protocol's escalation triggers.
...
- Broad implementation tasks must be split into bounded Operator phases with a phase plan approved before work starts; FullOperator implementation phases default to unbounded wall-clock runs (`-TimeoutSeconds 0`) unless intentionally timeboxed. `OPERATOR_VALIDATOR_PROTOCOL.md` owns the phase-bounded rule and timeout policy.
- When Claude is Operator for substantive implementation, Codex is primarily a wrapper/router, approval gate, and validator. Codex should forward the user's request plus the relevant task contract and repo rules to Claude instead of deeply planning the work itself. Codex must approve or reject the requested change scope before Claude makes changes. Approved Claude Operator runs may use the full Claude Code tool surface through `Scripts\Invoke-ClaudeDirectRead.ps1 -ToolProfile FullOperator`, including edits, shell commands, and configured MCP/editor tools, inside the Codex-approved scope.
- Proof-bearing work (tasks whose acceptance needs produced build logs, commandlet markers, runtime/editor captures, gameplay proof, visual judgment, or multiple proof classes) routes to FullOperator: a read-only packet may plan it but the implementation/proof phase must be a Codex-approved FullOperator run. Claude-produced proof is evidence, not final acceptance; Codex validates the actual evidence and stays the final reporter. `OPERATOR_VALIDATOR_PROTOCOL.md` owns the proof-routing and final-proof-owner rules.
- Operator artifacts and Operator-made changes are not greenlights. In the active Codex workspace, Codex remains responsible for the task contract, Codex approval, validation of actual changes, excluded operations, final verification review, and the user-facing completion report unless the user explicitly changes that.
...
|---|---|---|---|
| Validator review | Any substantive request unless the user opts out | Follow `OPERATOR_VALIDATOR_PROTOCOL.md`. The non-Operator model is the Validator unless the user explicitly names a different reviewer. The Validator must run the Packet Completeness Gate before discovery, return incomplete packets as `Verdict: REVISE`, and perform targeted anchor checks by default. When Claude validates Codex, use local Claude Code CLI through `Scripts\Invoke-ClaudePlanReview.ps1`; verify no `ANTHROPIC_API_KEY` in Process/User/Machine and do not use Anthropic API billing. | Report Operator, Validator, validation depth, greenlight artifact path when one exists, output scope, caveats, and token ledger data before completion. Strict first-line verdicts and fallback behavior are defined in `OPERATOR_VALIDATOR_PROTOCOL.md`. |
| Operator/Validator stack | User says `Make Claude operator` or `Make Codex operator`, asks to shift heavy work between Claude/Codex, conserve one model's usage, or use Claude as primary worker | Follow `OPERATOR_VALIDATOR_PROTOCOL.md` as the detailed authority. Read `.t66\operator-state.json` at task start when present, and update it through `Scripts\Set-T66Operator.ps1` for operator-switch commands. Use Claude Code `claude-opus-4-8` as the default heavy Operator when the state or user request selects Claude. When Claude is Operator, Codex should act as wrapper/router first: package the user's task and repo rules for Claude, avoid deep duplicate planning, approve or reject Claude's requested change scope, then validate the actual result. After approval, Claude may run full Operator mode. Quota denominator inference remains deferred until a separate usage-ledger/router pass is reviewed and implemented. | Report Operator, Validator, model, helper/profile, Codex approval artifact path, Operator artifact paths, verification, and phase-attributed token ledger. Operator artifacts and Operator-made changes are not greenlights unless Codex validates them under the protocol. |
| Claude tool access | Claude needs direct repo/file, shell, Blender, Unreal, Niagara, or editor visibility as Operator or Validator | Use `Scripts\Invoke-ClaudeDirectRead.ps1`. For Claude Operator implementation, use `-ToolProfile FullOperator` with a valid Codex approval artifact and non-plan permission mode. Full Operator mode may use edits, shell commands, and configured MCP/editor tools inside the approved scope. Use user-scoped Blender MCP only after smoke verification. Use Unreal-owned capture/dump scripts for Unreal/Niagara evidence; do not accept desktop screenshots or raw GUI observation as proof. | Claude helper artifact path, manifest `ClaudeTokensSpent`, Codex approval artifact path, actual-change validation, Blender MCP list/get output when used, and Unreal-owned artifact paths for visual/editor evidence. |
