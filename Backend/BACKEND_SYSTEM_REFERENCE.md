...
**Companion anti-cheat doc:** `Backend/Anti Cheat/ANTI_CHEAT_POLICY_REFERENCE.md`
**Historical predecessor docs:** deleted during the Alpha 0.1 documentation cleanup; recover from git history if needed.
**Maintenance rule:** Update this file after every Steam, backend, online, multiplayer, upload, deployment, or other internet-connected systems change. Update `Release/Steam/STEAMWORKS_UPLOAD_AND_TESTING_INSTRUCTIONS.md` in the same pass for any Steamworks/build/upload/private-test workflow change. If the change also affects project policy, workflow, or anti-cheat enforcement, update `Release/PROJECT_GUIDELINES_INSTRUCTIONS.md` and `Backend/Anti Cheat/ANTI_CHEAT_POLICY_REFERENCE.md` in the same pass.
**Status note:** For the current Steamworks operational state, active build ID, upload workflow, and private-testing procedure, prefer `Release/Steam/STEAMWORKS_UPLOAD_AND_TESTING_INSTRUCTIONS.md`. This backend document still contains historical transition details from the earlier `480` to `4464300` migration period.
...
- `Release/Steam/STEAMWORKS_UPLOAD_AND_TESTING_INSTRUCTIONS.md` is now the primary operator-memory file for Steamworks uploads, current build tracking, key handling, and the local PowerShell workflow.
