...
This file is the authoritative gameplay-world process for generating modular dungeon and difficulty-themed environment pieces for `T66`.
...
Historical sheet-based batch plan:

- `Model Generation/Runs/Environment/CoherentThemeKit01/Notes/batch_plan.md`
- The generated run folder is not a durable repo dependency. Recover it from git history or recreate it from the instructions here if the batch needs to be repeated.

Historical raw TRELLIS review target:

- `Model Generation/Runs/Environment/CoherentThemeKit01/Raw/Trellis`
...
Use the current model-generation decision tree in [00_MODEL_GENERATION_ROUTING_INSTRUCTIONS.md](C:/UE/T66/Model%20Generation/Instructions/00_MODEL_GENERATION_ROUTING_INSTRUCTIONS.md), the TRELLIS.2 RunPod baseline in [01_TRELLIS_RUNPOD_SETUP_INSTRUCTIONS.md](C:/UE/T66/Model%20Generation/Instructions/01_TRELLIS_RUNPOD_SETUP_INSTRUCTIONS.md), and the import/validation rules in [05_UNREAL_IMPORT_AND_VALIDATION_INSTRUCTIONS.md](C:/UE/T66/Model%20Generation/Instructions/05_UNREAL_IMPORT_AND_VALIDATION_INSTRUCTIONS.md).
