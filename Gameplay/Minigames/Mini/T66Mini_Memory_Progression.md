...
- Defined the intended implementation structure in `Gameplay/Minigames/Mini/T66Mini_MasterImplementation.md`.
...
- Added `Tools/ArtPipeline/Minigames/T66MiniSplitHeroSheets.py` and split those four sheets into 16 individual hero source sprites under `SourceAssets/Mini/Heroes/Singles/`.
...
- Added the current gap tracker document as `Gameplay/Minigames/Mini/T66Mini_GapChecklist.md` so implementation status can be checked against the master target without re-deriving the delta from code or chat history.
