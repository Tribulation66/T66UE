# T66 Master Optimization Audit V2

As of April 17, 2026.

This is the validated `V2` master optimization audit for `T66`, `T66Mini`, Steam/session plumbing, backend invite services, and shared infrastructure. It supersedes `T66_MASTER_OPTIMIZATION_AUDIT.md` as the current consolidated master. No gameplay code, UI code, backend code, or build-system code is changed in this phase.

## V2 Change Log

- Revalidated the original `V1` findings against the current repo and backend.
- Reviewed and validated the Claude refinement pass in `Audit/audit 4.16.26/T66_MASTER_OPTIMIZATION_AUDIT_Claude_Review.md`.
- Preserved the useful `V1` structure and findings so no good implementation guidance is stranded in the earlier file.
- Expanded the audit with three new areas:
...
- Expanded existing areas with validated Unreal-specific guidance for:
  - Slate invalidation and bound-attribute discipline
  - tick governance and pooling rules
  - profiling workflow and regression gates
- Corrected several overstatements from the Claude review:
  - invalidation is present in the repo, but not on the most important hot paths
  - instancing is present in the repo, but not consistently applied outside the strongest runtime systems
  - collision simplification is partly present already; the real issue is uneven policy and targeted misuse
  - Nanite should not be treated as a blanket yes/no answer here
  - GC tuning should follow measurement and pointer-safety audit, not precede them

## Validation Summary for the Claude Review

### Accepted

- Add a dedicated GC discipline section.
- Add a rendering/GPU/collision/instancing policy section.
- Add a networking/replication/relevancy section distinct from invite/join flow.
- Expand the frontend/UI sections with Slate-specific guidance.
- Expand the actor lifecycle section with stricter tick governance and pooling rules.
- Expand observability with a hypothesis-driven optimization workflow and explicit tool recipes.

### Refined Before Inclusion

- `UMG property binding` was reframed as `bound Slate attributes / lambda bindings on hot widgets`, because this repo is heavily C++ Slate-based and the code evidence is mainly `.Text_Lambda(...)` / `.Visibility_Lambda(...)`, not Blueprint designer bindings.
- `Invalidation is underused` was kept, but the claim that there is only one invalidation panel was corrected. The repo already has `SInvalidationPanel` usage in `T66MainMenuScreen` and `T66MiniMainMenuScreen`; the real gap is that the slowest/highest-traffic paths still do not lean on stable invalidation strategy.
- `No instancing` was corrected to `instancing exists, but policy is uneven`. The repo already uses `UInstancedStaticMeshComponent` in terrain, miasma, and some trap systems.
- `No NoCollision policy on props` was corrected. There are already many `NoCollision` and `CastShadow=false` usages; the real risk is that some terrain/prop systems still opt into `UseComplexAsSimple` and physics/query collision in places that need explicit profiling.
- `Nanite should be explicitly declined` was softened. Current hot paths are not pointing at a triangle-density bottleneck, and `r.Nanite.ProjectEnabled=True` is already present in config, so V2 treats Nanite as non-priority rather than forbidden.
- `Enable GC settings immediately` was corrected. Official Unreal guidance for incremental reachability requires safe pointer usage; V2 requires a `TObjectPtr` / ownership audit and a measured GC baseline before tuning project-wide GC CVars.

### Not Promoted to V2 as Canonical Rules

- Hard-coded draw-call targets taken from non-Epic sources.
- Blanket HISM-over-ISM rules.
- `stat memgc` as a canonical required command.
- Architecture-heavy custom widget rendering as a first-line fix before simpler lifecycle/loading wins are exhausted.

## Scope and Source of Truth

This audit is grounded first in the current repo and backend. Existing audits and external articles are supporting inputs, not authority over the code. Where article advice conflicted with current repo reality or official Unreal guidance, `V2` keeps the repo-grounded version.
...
Primary external validation sources used for `V2`:
...
The backend invite routes are comparatively thin. They authenticate, read/write invite state, and return lightweight responses. That is good news, because it means the multi-second feel problem is not primarily a heavy route-handler problem.
...
- Extend existing instancing patterns beyond terrain/traps where repeated decor or prop actors remain individual.
...
`V1` was directionally correct. The Claude review added several important areas that deserved inclusion. The most useful `V2` refinement is not "more recommendations"; it is **better filtering**:

- keep the concrete repo-grounded feel killers from `V1`
- add the missing infrastructure topics from the review
...
The problem remains inconsistency. `V2` keeps the original thesis: the game feels janky not because one system is catastrophically wrong, but because multiple important paths still mix modern async/event-driven patterns with older rebuild-centric, sync-fallback, scan-heavy, and spawn-heavy behavior.

That is the right thing to fix before the art pass.

## Appendix A: Official Reference Index

Primary Unreal references used to validate `V2`:
...
These were useful as secondary inputs during refinement, but `V2` only promoted ideas from them when they aligned with repo evidence or official Unreal guidance.
