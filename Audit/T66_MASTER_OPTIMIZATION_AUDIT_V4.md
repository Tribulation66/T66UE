# T66 Master Optimization Audit V4

As of April 17, 2026.

This is the validated `V4` master optimization audit for `T66`, `T66Mini`, Steam/session plumbing, backend invite services, and shared infrastructure. It supersedes `T66_MASTER_OPTIMIZATION_AUDIT_V3.md` as the current consolidated master. No gameplay code, UI code, backend code, or build-system code is changed in this phase.

## V4 Change Log

- Revalidated the current `V3` master against the repo, config, and the Batch 3 article set targeted at T66's actual stack.
- Preserved the full `V3` structure and implementation guidance so no high-value findings are stranded in the earlier masters.
- Added validated refinements for:
  - bounded `ProceduralMeshComponent` terrain-surface guidance in `Area 8` and `Area 15`
  - projectile, telegraph, and transient-VFX pooling parity in `Area 6`, `Area 9`, `Area 10`, and `Area 16`
  - Niagara system-budget, scalability, and profiling rules in `Area 9` and `Area 13`
  - Mini projectile replication and quantization guidance in `Area 16`
  - always-cook scope review for heavy content directories in `Area 15`
  - additional official and supplementary references in the appendices
- Corrected the main overstatements from the Batch 3 article pass before merging them:
  - the `ProceduralMeshComponent` issue in `T66MainMapTerrain` is real, but it is a bounded surface-feature path, not a blanket "rewrite the terrain system" verdict
  - the lack of `HISM` in the repo is not automatically a bug; batched ISM add/update discipline remains the more urgent local issue
  - `Stylized_VFX_StPack` and `UE5RFX` are not dead sample folders; both have live runtime references, so cook-scope work must narrow them, not blindly remove them
  - flow fields, `RealtimeMeshComponent`, and Nanite/VSM changes remain measure-first options, not canonical fixes
...
### Not Promoted to V4 as Canonical Rules
...
- Epic documentation and blogs on replication performance and Virtual Shadow Maps
...
- Selected runtime mesh, pooling, Niagara, and crowd-pathfinding articles used only when they aligned with repo evidence
...
The HUD and VFX systems already show optimization awareness, but there is still too much frame-driven presentation work and too little explicit Niagara budgeting. HUD tick, bound Slate attributes, repeated material parameter writes, per-enemy widget components, sync VFX warmup, and transient Mini VFX actors are all contributing factors.
...
9. Extend pooling discipline to repeated combat projectiles, telegraphs, and transient VFX where captures show live spawn/destroy cost.
10. Narrow always-cook scope for large content directories before the art pass grows payload further.
...
8. Run a measured Nanite/VSM and translucent-VFX review by asset class rather than by blanket engine setting.
...
`V1` was directionally correct. The Claude review passes added several useful areas, but the biggest improvement through `V4` is still **better filtering**, not volume:

- keep the concrete repo-grounded feel killers from `V1`
- preserve the broader infrastructure topics from `V2`
- add the validated Steam, backend, instancing, PMC, Niagara, replication, and cook-scope rules from the later passes
...
The problem remains inconsistency. `V4` keeps the original thesis: the game feels janky not because one system is catastrophically wrong, but because multiple important paths still mix modern async/event-driven patterns with older rebuild-centric, sync-fallback, scan-heavy, spawn-heavy, and over-broad content/cook behavior.

`V4` keeps that thesis and adds the supporting guardrails that stop future work from backsliding:
...
- treat bounded PMC, Niagara, projectile replication, and always-cook scope as measurable engineering contracts rather than as places to copy generic Unreal advice blindly

That is the right thing to fix before the art pass.

## Appendix A: Official Reference Index

Primary official references used to validate `V4`:
...
These were useful as secondary inputs during refinement, but `V4` only promoted ideas from them when they aligned with repo evidence or official guidance.
