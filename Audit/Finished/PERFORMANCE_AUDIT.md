...
This audit is now closed against the current source tree.

- The previously unchanged `AT66EnemyDirector` spawn-wave concern is implemented: `T66EnemyDirector` now builds `PendingSpawns`, processes them through `SpawnNextStaggeredBatch()`, and caps each materialized batch with `MaxSpawnsPerStaggeredBatch`.
- The secondary wheel-spin concern is no longer a 60 Hz loop in the current HUD path: `FT66HUDPresentationController::StartWheelSpin()` schedules `TickWheelSpin()` at `0.033f` seconds.
- The older hot `TActorIterator` issues named in this file have been replaced by caches, throttles, the actor registry, or staggered queues in the cited current source paths.

Remaining performance work exists in the broader optimization audits, but not as an open item in this older audit.
...
| 2.2 | EnemyDirector SpawnWave | **FIXED** | Runtime waves are queued in `PendingSpawns` and materialized through staggered batches capped by `MaxSpawnsPerStaggeredBatch`. |
...
Secondary issues are also closed in the current source: wheel spin runs at ~30 Hz in `FT66HUDPresentationController`, and `EnemyDirector` runtime waves are staggered through `PendingSpawns`.
...
### 2.2 AT66EnemyDirector::SpawnWave — **FIXED**

**File:** `Source/T66/Gameplay/T66EnemyDirector.cpp` (lines 107–250+)

- **What:** Runtime wave planning can still choose a large `EffectivePerWave`, but current source no longer spawns the whole plan in one frame.
- **Impact:** The original hitch risk is mitigated by queuing the planned spawns in `PendingSpawns`.
- **Fix applied:** `SpawnRuntimeTrickleWave()` builds queued spawn slots, then `SpawnNextStaggeredBatch()` materializes only `MaxSpawnsPerStaggeredBatch` entries per batch with `StaggeredSpawnIntervalSeconds` between batches.
...
- **Wheel spin presentation:** current HUD presentation schedules `TickWheelSpin` at 0.033f (~30 Hz) — acceptable.
...
- **EnemyDirector runtime wave:** SpawnIntervalSeconds (e.g. 2 s) — planned waves are staggered through `PendingSpawns`.
...
7. ~~**EnemyDirector::SpawnWave**~~ — **Done:** runtime waves now use `PendingSpawns` plus `SpawnNextStaggeredBatch()`.
...
| T66HUDPresentationController.cpp | Wheel spin timer 0.033f |
