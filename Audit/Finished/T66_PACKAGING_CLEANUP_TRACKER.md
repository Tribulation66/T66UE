...
## Verified finished status (updated after 2026-05-05 remediation)

This tracker is finished from the automated packaging/runtime asset-contract side. The 2026-05-05 remediation added a config-backed loose-runtime content policy, made the guard prove that policy against `T66.Build.cs`, removed the stale loose outer-wall dependency that no longer has a runtime caller, and validated a fresh full-cook staged standalone.
...
- `SourceAssets/OuterWallTexture.png` is no longer staged as a loose runtime dependency; current source only references it from the editor/import setup script, not runtime code.
- `UT66GameContentSettings` now owns the central loose-runtime content policy in config, with each staged loose root carrying an owner, classification, and rationale.
- `Scripts/GuardT66RuntimeAssetContract.ps1` now checks owned core sync-load exposure, duplicate loose runtime dependency declarations, duplicate/stale/incomplete policy entries, missing staged roots, and one-to-one coverage between `Source/T66/T66.Build.cs` and `Config/DefaultGame.ini`.
- The guard passes with `-FailOnDuplicateLooseRuntimeDependencies` and reports six intentional loose roots: `RuntimeDependencies/T66/Fonts/...`, `RuntimeDependencies/T66/UI/...`, `SourceAssets/Arcade/...`, `SourceAssets/UI/HeroSelection/...`, `SourceAssets/UI/Reference/...`, and `SourceAssets/UI/PowerUp/Diplomas/...`.
- The core `UT66GameInstance` DataTable getter helper no longer performs a direct fallback `LoadSynchronous()` and instead primes/waits on the existing async core-table load handle.
- `Config/DefaultScalability.ini` and `Config/DefaultDeviceProfiles.ini` now exist and pass a fresh cook. The first cook failed on invalid non-scalability CVars, and the final file was corrected to only use UE scalability CVars.
- Fresh packaged validation passed: `Scripts/StageStandaloneBuild.ps1 -ClientConfig Development` completed a full cook/package/stage and produced `C:\UE\T66\Saved\StagedBuilds\Windows\T66\Binaries\Win64\T66.exe`.
- Follow-up packaged validation passed: `Scripts\StageStandaloneBuild.ps1 -ClientConfig Development -SkipCook` refreshed the same staged standalone after the final code-only trap diagnostic adjustment.

Reference-only follow-up notes:

- `SavePreview` is treated as intentionally restored current UX, not as a cleanup regression against this tracker.
- Runtime manual verification remains appropriate for save/load resume behavior, media viewer stability, Steam invite/join, and real gameplay/Mini feel. The current automated packaged stage is clean, but those user-flow checks still need a live run.
- The remaining loose runtime/reference-art surfaces are intentional and guard-covered, not unclassified staging drift. Future cooked-texture migration can remove them, but the current package contract is explicit and non-duplicate.
...
| 4 | Asset reference centralization design | Shared runtime asset catalog / path-owner layer | Scattered high-risk asset strings have a clear future owner | Complete for packaging scope - `UT66GameContentSettings` now owns staged loose-runtime root policy |
| 5 | Raw file-system dependency quarantine | Explicit inventory of raw-disk runtime dependencies with owners | Every raw-disk runtime dependency is isolated, classified, or scheduled for removal | Complete for current staged roots - six loose roots are classified and guard-covered |
| 6 | Runtime data determinism cleanup | CSV/DataTable override and loose data-path inventory | Import-time data flow separated from runtime data flow | Complete for packaging scope - core DataTable fallback is async-handle based and loose data roots are policy-owned |
| 7 | Dynamic discovery inventory | Asset registry / folder-discovery risk map | Dynamic discovery systems have documented cook assumptions | Complete for packaging scope - staged loose roots and cook-sensitive assumptions are covered by config policy plus guard checks |
| 8 | Config and module hygiene | Packaging-relevant config and dependency audit | Stale packaging assumptions and accidental runtime/editor coupling documented | Complete - scalability/device profiles added, `DeveloperSettings` dependency added, stale outer-wall staging removed, and full cook/stage validated |
| 9 | Logging and fallback cleanup | Missing-asset/missing-data observability plan | Runtime fallbacks are explicit and diagnosable | Complete for current package - guard diagnostics identify sync-load exposure, stale policy entries, missing staged roots, incomplete metadata, and duplicate/unclassified loose roots |
| 10 | Cleanup guardrails | Audit script / grep rules and guideline update inputs | Cleanup regressions can be detected automatically | Complete for asset-contract scope - guard validates duplicate, stale, missing, and unclassified loose runtime roots |
...
Historical runtime data determinism risks from the original Pass 6 check:

- `Source/T66/Core/T66GameInstance.cpp` previously hot-overrode cooked DataTables from raw CSV files at runtime:
  - `Content/Data/Idols.csv`
  - `Content/Data/Bosses.csv`
  - `Content/Data/Stages.csv`
- The override helper used `ProjectContentDir()` plus `FFileHelper::LoadFileToString`, so packaged parity depended on loose files being present.
- `Source/T66/Core/T66LeaderboardSubsystem.cpp` previously treated CSVs as the live runtime source of truth by loading CSVs before DataTables.
- `Source/T66/Core/T66LeaderboardSubsystem.h` previously pointed `ScoreTargetsDTPath` at the wrong `DT_Leaderboard_ScoreTargets` asset path.
- `SpeedRunTargetsDTPath` matched a real asset: `DT_Leaderboard_SpeedrunTargets.uasset`.

Current verification on 2026-05-04:

- The named `T66GameInstance.cpp` CSV hot-override paths are no longer present in current source.
- `UT66LeaderboardSubsystem::Initialize()` now calls `LoadTargetsFromDataTablesIfPresent()` and `ScoreTargetsDTPath` points at `/Game/Data/Leaderboard_ScoreTargets.Leaderboard_ScoreTargets`.
- The remaining concern is not this specific leaderboard path; it is the broader packaged-runtime asset/data contract, including residual loose helper surfaces and synchronous fallback loads.

Cleanup consequence:

- The original leaderboard path mismatch and CSV-first leaderboard loading are closed in current source.
- Keep this pass open only for broader packaged-runtime data/asset determinism follow-up, not for the old `ScoreTargetsDTPath` defect.
...
Historical findings from the original pre-audit:

- `Source/T66/Core/T66GameInstance.cpp` actively overrode cooked DataTables from raw CSV files at runtime for:
  - `Content/Data/Idols.csv`
  - `Content/Data/Bosses.csv`
  - `Content/Data/Stages.csv`
- `Source/T66/Core/T66LeaderboardSubsystem.cpp` loaded CSV first in `Initialize()` and only fell back to DataTables if CSV population failed.
- `Source/T66/Core/T66LeaderboardSubsystem.h` pointed `ScoreTargetsDTPath` at `/Game/Data/DT_Leaderboard_ScoreTargets`, while the cooked asset present on disk was `Content/Data/Leaderboard_ScoreTargets.uasset`.

Current verification on 2026-05-04:

- The named `T66GameInstance.cpp` CSV hot-override paths are no longer present.
- `Source/T66/Core/T66LeaderboardSubsystem.cpp` now loads cooked DataTables first through `LoadTargetsFromDataTablesIfPresent()`.
- `Source/T66/Core/T66LeaderboardSubsystem.h` now points `ScoreTargetsDTPath` at `/Game/Data/Leaderboard_ScoreTargets.Leaderboard_ScoreTargets`.

Cleanup consequence:

- The old CSV-first leaderboard issue is no longer pending.
- Remaining validation should focus on packaged/runtime parity for the current cooked DataTable flow and any residual loose data helpers.
...
- The staged `RuntimeDependencies/T66/...` directory still exists and is staged by `Source/T66/T66.Build.cs`.
- The legacy helper names above have since been replaced by `T66RuntimeUIFontAccess` and `T66RuntimeUITextureAccess` in current source.
