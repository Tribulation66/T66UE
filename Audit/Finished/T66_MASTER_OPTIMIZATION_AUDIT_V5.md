...
## Verified finished status (updated after 2026-05-05 remediation waves)

This audit is no longer an active `Pending` fix queue. The 2026-05-04 and 2026-05-05 remediation waves closed the source-confirmed blockers that kept V5 pending, and the remaining notes are either live/manual validation or longer-term reference backlog.

Closed or materially resolved in current source:

- The older `PERFORMANCE_AUDIT.md` items remain closed, including staggered `AT66EnemyDirector` runtime spawning through `PendingSpawns` / `SpawnNextStaggeredBatch()`.
- Runtime data determinism is improved: leaderboard score targets use `/Game/Data/Leaderboard_ScoreTargets.Leaderboard_ScoreTargets`, the leaderboard subsystem initializes from cooked DataTables instead of CSV-first, TD required tuning rows no longer silently fall back to per-call C++ literals, and Mini runtime tuning callers no longer use defaulted `GetRuntimeTuningValue(..., <literal>)` fallback paths.
- The runtime asset contract is guardable: `Scripts/GuardT66RuntimeAssetContract.ps1 -FailOnDuplicateLooseRuntimeDependencies` reports `Owned Core sync-load exposure lines: 0`, six loose runtime dependency declarations, and six matching policy entries.
- `UT66GameContentSettings` now owns the loose-runtime content policy in config. Each staged loose root has an owner, classification, and rationale, and stale loose staging such as `SourceAssets/OuterWallTexture.png` has been removed.
- Main trap meshes/VFX are no longer scattered constructor fallback loads; trap asset paths are config-owned under `TrapAssets.*` and resolved through async/resident runtime paths.
- Combat/Boss/Mini VFX now have pooling, per-frame caps, and scalability-aware budget knobs for the audited projectile, AOE, plague cloud, unique debuff, stage-effect, and Mini transient VFX paths.
- Main and Mini nearest-player lookup hot paths now use cached/registered lookup paths instead of the cited per-frame per-enemy scan shape.
- The broad main-runtime `TActorIterator` cleanup was narrowed to current-source evidence: repeated hot scans were replaced or cached, and the remaining sites are setup, cache-miss recovery, stage transition, teardown, or dev-audit surfaces rather than active per-frame blockers.
- `UT66ScreenBase::ForceRebuildSlate()` now routes through a coalesced deferred rebuild helper, so existing screen calls no longer imply immediate repeated widget teardown/rebuild work.
- Healthy direct invite/lobby joins no longer use the friend-session fallback retry policy; fallback retry remains only for the explicit friend-lookup fallback path and logs retry policy/reason/delay.
- The cited `T66WebImageCache` raw `this` HTTP/game-thread callbacks are weak-gated and deinitialize-aware, and the cited backend account, leaderboard, run, and run-summary JSON parser paths use guarded field reads instead of unsafe `Get*Field` assumptions.
- `Config/DefaultScalability.ini` and `Config/DefaultDeviceProfiles.ini` exist and survived fresh cook after removing invalid non-scalability CVars from the scalability file.
- `SavePreview` is treated as intentionally restored current UX, not as a packaging cleanup blocker.

Validation completed after the remediation waves:
...
- `Scripts/StageStandaloneBuild.ps1 -ClientConfig Development`: passed with a fresh full cook/package/stage, producing `C:\UE\T66\Saved\StagedBuilds\Windows\T66\Binaries\Win64\T66.exe`.
- `Scripts\StageStandaloneBuild.ps1 -ClientConfig Development -SkipCook`: passed after the final code-only trap diagnostic adjustment, refreshing the same staged standalone executable.

Reference-only follow-up notes:

- Live/manual validation is still the right place to verify Steam-to-Steam invite timing, save/load resume behavior, media viewer stability, real gameplay HUD/combat feel, and Mini combat feel.
- Broader non-core sync/import helpers, primary asset registry expansion, CommonUI/global invalidation strategy, and full content migration away from loose reference art remain useful future reference topics, but they are not active blockers for this audit's current remediation queue.
