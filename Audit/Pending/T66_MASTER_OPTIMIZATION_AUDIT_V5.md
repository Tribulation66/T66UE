...
This audit remains pending, but many of the original concrete problems are already solved in current source.

Resolved or materially improved in current source:

- The older `PERFORMANCE_AUDIT.md` items are closed, including staggered `AT66EnemyDirector` runtime spawning through `PendingSpawns` / `SpawnNextStaggeredBatch()`.
- Runtime data determinism is improved: leaderboard score targets now use `/Game/Data/Leaderboard_ScoreTargets.Leaderboard_ScoreTargets`, and the leaderboard subsystem initializes from cooked DataTables instead of CSV-first.
- Packaging fallback ownership is cleaner: `RuntimeDependencies/T66/...` is staged, `T66DotaSlate.*` is gone, and `T66RuntimeUITextureAccess` owns the current loose/reference-art loading path.
- Several packaging cleanup slices named in the tracker exist in source: lighting subsystem extraction, combat VFX split, media viewer debounce/idempotent show state, actor-registry replacement for the cited HUD/enemy hot scans, and 30 Hz wheel-spin presentation.

Still pending against current source:

- Main runtime still has nontrivial `TActorIterator` / `GetAllActorsOfClass` usage outside the already-fixed HUD/enemy hot paths, so the broad "world-scan cleanup" recommendation is not closed.
- Sync-load exposure remains in current source (`LoadSynchronous`, `TryLoad`, `LoadClass`, and related fallback paths), so the async-ready asset contract is not closed.
- Hot UI still has many `ForceRebuildSlate`, `TAttribute`, `Visibility_Lambda`, and active-timer surfaces; the global-invalidation/invalidation-policy work is still pending.
- Mini still uses replicated movement on several actor classes and still has screen rebuild / runtime churn surfaces, so Mini replication/relevancy and runtime-hygiene work remains pending.
- Current source has a restored `SavePreview` screen/path, which conflicts with the packaging tracker's earlier "preview removal" cleanup pass unless that restoration is now intentional.
- Current packaged/manual validation is still missing for Steam invite join, real gameplay HUD/combat feel, Mini runtime feel, lighting, combat VFX, save/load resume, and media viewer stability.
