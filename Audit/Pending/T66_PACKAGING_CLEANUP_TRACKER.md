...
## Verified pending status (2026-05-04)

This tracker is still pending, but the pending surface is much narrower than the older pass notes imply.
...
- `RuntimeDependencies/T66/...` exists and is staged through `Source/T66/T66.Build.cs`.
...
Still pending after checking current source:

- Save-preview removal is no longer true. `ET66ScreenType::SavePreview`, `UT66SavePreviewScreen`, registration in `T66PlayerController_*`, and a preview button in `T66SaveSlotsScreen` are present again. Decide whether this is intentionally restored UX or a regression against the cleanup plan.
- Runtime verification remains open for lighting extraction, combat VFX extraction, save/load resume behavior, and media viewer stability; the tracker records build success, but not fresh current packaged/manual validation for those flows.
- There are still loose runtime/reference-art surfaces: `T66.Build.cs` stages `RuntimeDependencies/T66/...` plus `SourceAssets/UI/Reference/...`, `SourceAssets/UI/HeroSelection/...`, and other loose UI folders. That may be intentional for the current generated-UI workflow, but it is not the strict cooked-only asset contract originally targeted.
- Sync-load exposure still exists in current source (`LoadSynchronous`, `TryLoad`, and `LoadClass` paths remain), so the packaged-runtime asset-resolution audit is not fully closed.
