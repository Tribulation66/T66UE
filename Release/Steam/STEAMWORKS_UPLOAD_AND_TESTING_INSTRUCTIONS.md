...
**Companion docs:** `Backend/BACKEND_SYSTEM_REFERENCE.md`, `Release/PROJECT_GUIDELINES_INSTRUCTIONS.md`  
**Maintenance rule:** Update this file after any Steamworks change, Steam build upload, branch/live-build switch, key request, release-state change, or multiplayer-validation milestone. If the change also affects backend auth/routes, update `Backend/BACKEND_SYSTEM_REFERENCE.md` in the same pass.
...
- `C:\UE\T66\Tools\Release\Steam\UploadToSteam.ps1`
...
- `C:\UE\T66\Backend\BACKEND_SYSTEM_REFERENCE.md`
...
```powershell
powershell -ExecutionPolicy Bypass -File "C:\UE\T66\Tools\Release\Steam\UploadToSteam.ps1" `
...
2. read `Backend/BACKEND_SYSTEM_REFERENCE.md` second
3. verify local `appmanifest_4464300.acf`
4. verify the current branch/tag/working tree
5. prefer `Tools/Release/Steam/UploadToSteam.ps1` over manual ContentBuilder copying
...
- `Tools\Release\Steam\UploadToSteam.ps1` now removes local-only `steam_appid.txt` from the ContentBuilder content root before upload, because SteamPipe warned that file should not be included in depots.
- SteamCMD used cached credentials for `tribulation66` and completed `run_app_build` successfully.
...
- Historical local packaged capture for the pass was removed with the old generated UI output tree during Alpha 0.1 cleanup; recover it from `v4.1` only if that proof image is needed.
