...
INNER_LINES_DEFAULT_BLACK = "/Game/ToonStyle/Textures/T_InnerLines_DefaultBlack"
...
def get_material_vector_parameter(material_path: str, parameter_name: str) -> list[float] | None:
    asset = unreal.EditorAssetLibrary.load_asset(material_path)
    if not isinstance(asset, unreal.MaterialInstanceConstant):
        return None
    try:
        value = unreal.MaterialEditingLibrary.get_material_instance_vector_parameter_value(asset, parameter_name)
    except Exception as exc:
        unreal.log_warning(f"[ImportPixal3DAsset] Could not read {parameter_name} from {material_path}: {exc}")
        return None
    if not value:
        return None
...
def import_texture(source_path: Path, texture_dir: str, texture_name: str, lod_group_name: str) -> tuple[str, unreal.Texture2D]:
    ...
    for prop_name, value in settings.items():
        try:
            ...
            unreal.log_warning(f"[ImportPixal3DAsset] Could not set texture {prop_name} on {texture_path}: {exc}")
...
def create_material(
    ...
    tint_texture: unreal.Texture2D | None = None,
    inner_line_texture: unreal.Texture2D | None = None,
    outline_color: list[float] | None = None,
...
def main() -> int:
    ...
    previous_outline_color = get_material_vector_parameter(outline_material_path, "OutlineColor")
    ...
    tint_texture_asset = None
    tint_texture_path = None
    ...
    if tint_source.exists():
        tint_texture_path, tint_texture_asset = import_texture(
            tint_source,
            texture_dir,
            f"T_{args.asset_name}_Tint",
            "TEXTUREGROUP_CHARACTER",
        )

    inner_line_texture_asset = unreal.EditorAssetLibrary.load_asset(INNER_LINES_DEFAULT_BLACK)
    inner_line_texture_path = INNER_LINES_DEFAULT_BLACK if inner_line_texture_asset else None
    ...
    if inner_source.exists():
        inner_line_texture_path, inner_line_texture_asset = import_texture(
            inner_source,
            texture_dir,
            f"T_{args.asset_name}_InnerLines",
            "TEXTUREGROUP_CHARACTER",
        )
    ...
    verify = {
        ...
        "tint_texture": tint_texture_path,
