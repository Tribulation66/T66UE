...
- Mini: `SourceAssets/Mini`, `Content/Mini`, `Docs/Mini`
- TD: `SourceAssets/TD`, `Content/TD`, `Docs/TD`
- Deck: `SourceAssets/Deck`, `Content/Deck`, `Docs/Deck`
- Idle: `SourceAssets/Idle`, `Content/Idle`, `Docs/Idle`

Shared helper scripts should live under `Tools/Minigames` when they become mode-neutral. Keep mode-specific import or runtime wiring in the owning mode.
...
Do not globally delete all green-ish pixels. The existing Mini split tooling already has a useful model in `Tools/Mini/T66MiniSplitSheets.py` through `remove_edge_connected_background`; a shared helper should move that idea into `Tools/Minigames`.
...
```powershell
python .\Tools\Minigames\T66MinigamePixelSnap.py `
...
```powershell
python .\Tools\Minigames\T66MinigameValidateSpriteAtlas.py `
...
```powershell
python .\Tools\Minigames\T66MinigameExportAnimationPreviews.py `
...
- add `Tools/Minigames/T66MinigamePixelSnap.py`
- add `Tools/Minigames/T66MinigameValidateSpriteAtlas.py`
- add `Tools/Minigames/T66MinigameExportAnimationPreviews.py`
...
The current [T66Mini_WalksheetPipeline.md](C:/UE/T66/Docs/Mini/T66Mini_WalksheetPipeline.md) remains the lightweight Mini-specific process for one 2x2 walk candidate.
