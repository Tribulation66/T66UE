...
Use the scaffold script before starting a new screen pack so the layout master, hi-res art master, prompt pack, and runtime family folders all start from the same structure.

## Current Main Menu Pipeline

1. Use the main menu reference screenshot as the only visual source of truth.
...
5. Generate isolated component boards by family instead of one giant all-in-one sheet.
...
10. If the original reference already contains visible buttons or labels inside the background plate, generate buttonless/background-clean variants before final runtime integration.
...
- For the anchor screen, do not force bad manual cleanup artifacts into runtime just because they exist. If a family is visibly wrong in-engine, generate a tighter family-specific replacement board and promote that board to the source pack.
- When extracting a family from the anchor screen, explicitly say whether the request is for:
  ...
  - buttonless background reconstruction
...
- Compare the rebuilt screen against the original reference screenshot at the same resolution.
